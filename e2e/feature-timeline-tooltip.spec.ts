...
import type {
  ...
  RevisionId,
  SketchId,
} from "../src/contracts/shared/ids";
import {
  CONTRACT_VERSION,
  EXTRUDE_FEATURE_SCHEMA_VERSION,
} from "../src/contracts/shared/versioning";
...
test("feature error timeline tooltip remains fully visible above the history bar", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);

  await workbench.openWithOperationHistory(
    createRepairableFeatureErrorHistory(),
  );

  const errorFeatureButton = page
    .locator('button[data-feature-error="true"]')
    .first();
  ...
  await expect(tooltip).toContainText(
    "Edit Repairable Profile and choose a valid profile selection.",
  );

  const geometry = await page.evaluate(() => {
    const tooltipElement =
      document.querySelector<HTMLElement>('[role="tooltip"]');
    const errorButtonElement = document.querySelector<HTMLElement>(
      'button[data-feature-error="true"]',
    );
    ...
    if (!tooltipElement || !errorButtonElement || !historyScrollerElement) {
      throw new Error(
        "Expected tooltip, error feature button, and history scroller to be present.",
      );
    }
    ...
    return {
      ...
      historyScrollerContainsTooltip:
        historyScrollerElement.contains(tooltipElement),
      historyScrollerOverflowY: getComputedStyle(historyScrollerElement)
        .overflowY,
  ...
  expect(geometry.tooltip.right).toBeLessThanOrEqual(
    geometry.viewport.width + tolerance,
  );
  expect(geometry.tooltip.bottom).toBeLessThanOrEqual(
    geometry.viewport.height + tolerance,
  );
  expect(geometry.tooltip.bottom).toBeLessThanOrEqual(geometry.button.top - 4);
  expect(geometry.tooltip.bottom).toBeLessThanOrEqual(
    geometry.historyScroller.top - 4,
  );
  expect(geometry.historyScroller.scrollHeight).toBeLessThanOrEqual(
    geometry.historyScroller.clientHeight + tolerance,
  );
});

test("history timeline hide toggle remains visible above the document tab bar", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
  ...
  const geometry = await page.evaluate(() => {
    ...
    if (!toggleElement || !tabBarElement) {
      throw new Error(
        "Expected the timeline toggle and document tab bar to be present.",
      );
    }
