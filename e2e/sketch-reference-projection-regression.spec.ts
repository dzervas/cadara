...
test("reload preserves a circle centered on a projected solid vertex", async ({
  page,
}, testInfo) => {
  ...
  await expect(page.getByText("Vertex Center Circle").first()).toBeVisible({
    timeout: 30_000,
  });
  await expect
    .poll(() => page.locator("body").textContent(), { timeout: 30_000 })
    .not.toContain("replay-exception");
...
test("reopen and finish preserves a face-backed referenced-vertex circle sketch", async ({
  ...
  await expect(page.getByText("Vertex Center Face Circle").first()).toBeVisible(
    { timeout: 30_000 },
  );
