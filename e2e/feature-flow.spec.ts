...
test("extrude previews and commits from the shared feature harness", async ({
  page,
}) => {
...
test("revolve previews and commits with a planar profile and durable edge axis", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("fillet previews and commits from a durable body edge", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("chamfer previews and commits from a durable body edge", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("thicken previews and commits from a durable planar face", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("combine subtract previews, commits, and persists consumed tool output", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("delete-solid previews and commits from an explicit body target", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("mirror previews and commits from explicit body and plane references", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("transform previews and commits from explicit body and reference selections", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("shell previews, commits, and keeps consecutive canvas frames stable", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("plane previews and commits from a planar face reference", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("extrude boolean scope previews and commits with an explicit target body", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("extrude preselects join and target body for a coplanar face preview", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("extrude preserves manual operation and target after later preselection changes", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("feature chain carries durable context across feature steps", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
