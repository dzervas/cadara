...
function createRectangleSketchDefinition(
  ...
  const bottomLeftPointId =
    `sketch_point_${idPrefix}_rect-bottom-left` as const;
  const bottomRightPointId =
    `sketch_point_${idPrefix}_rect-bottom-right` as const;
  ...
  return {
    ...
    dimensionIds: [
      `dimension_${idPrefix}_width`,
      `dimension_${idPrefix}_height`,
    ],
    dimensions: [
      {
...
function deriveFixtureRegionId(
  ...
  const definition = createRectangleSketchDefinition(
    sketchId,
    idPrefix,
    bounds,
  );
  ...
  if (!region) {
    throw new Error(
      `Fixture sketch ${sketchId} did not produce a profile region.`,
    );
  }
    ...
    entries: [
      ...
      createCreateFeatureHistoryEntry(
        createExtrudeFeatureRequest(
          "sketch_2",
          SECONDARY_EXTRUDE_FIXTURE.regionId,
        ),
      ),
    ],
  };
