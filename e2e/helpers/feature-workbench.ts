...
export class FeatureWorkbenchHarness extends SketchWorkbenchHarness {
  ...
  override async open(path = "/") {
    ...
    await expect
      .poll(() => this.revisionLabel(), { timeout: 30_000 })
      .not.toBe("loading");
  ...
  async selectReference(targetId: string) {
    ...
    await expect
      .poll(() => this.currentEditorSelection(), { timeout: 10_000 })
      .toContain(targetId);
  }
  ...
  async selectSupportedLoftFaceProfile() {
    ...
    await expect
      .poll(() => this.featureSessionLabel(), { timeout: 10_000 })
      .toContain("create:loft:");
    const faceTargets = await this.currentSelectableTargetsMatching(
    ...
    if (!firstFaceTarget || !secondFaceTarget) {
      throw new Error(
        "Expected the base extrude fixture to expose at least two selectable face targets.",
      );
    }
    ...
    await expect
      .poll(
        async () => {
          ...
          return session.includes("create:loft:previewReady")
            ? "ready"
            : session || "pending";
        },
        { timeout: 10_000 },
  ...
  async expectOperationSelected(
    ...
    ).toHaveValue(operationLabels[operation], { timeout: 10_000 });
  }

  async expectBooleanTargetSelected(bodyId: string) {
    await expect(this.featureInspector()).toContainText(
      `Target body: ${bodyId}`,
      { timeout: 10_000 },
    );
  }
  ...
  async expectNoBooleanTargetSelected() {
    await expect(this.featureInspector()).not.toContainText("Target body:", {
      timeout: 10_000,
    });
  }
  ...
  async flipDepthDirection() {
    await this.page
      .getByRole("button", { name: "Flip Depth direction" })
      .click();
  }
  ...
  async expectFeaturePreviewReady(kind: FeatureKind) {
    await expect
      .poll(() => this.featureSessionLabel(), { timeout: 30_000 })
      .toContain(`create:${kind}:previewReady`);
    await expect
      .poll(() => this.currentPreviewDiagnosticsText(), { timeout: 10_000 })
  ...
  async commitFeature(expectedFeatureId?: string) {
    ...
    await expect
      .poll(() => this.machineLabel(), { timeout: 75_000 })
      .toContain("idle");
            ...
            this.evaluateWithNavigationRetry(
              (featureId) =>
                window.__cadaraDebug
                  ?.getState()
                  ?.featureIds.includes(featureId) ?? false,
              expectedFeatureId,
            ),
          { timeout: 30_000 },
        )
  ...
  async selectReferenceByViewportPicking(targetId: string) {
    ...
    if (!viewportPoint) {
      throw new Error(
        `No projected viewport point is available for ${targetId}.`,
      );
    }
  ...
  async expectBodyPresent(bodyId: string) {
    await expect
      .poll(() => this.listVisibleBodyIds(), { timeout: 30_000 })
      .toContain(bodyId);
  }
  ...
  private async selectReferenceThroughCurrentUi(targetId: string) {
    ...
    if (sidebarLabel) {
      await this.page
        .locator("aside")
        .getByRole("button", { name: sidebarLabel })
        .first()
        .click();
      return;
    }
  ...
  private async selectOperation(operation: string) {
    ...
    const operationField = this.page.getByRole("combobox", {
      name: "Operation",
    });
    const currentValue =
      (await operationField.textContent())?.trim().toLowerCase() ?? "";
  ...
  private async currentSelectableTargetsMatching(pattern: RegExp) {
    return this.evaluateWithNavigationRetry(
      ({ source, flags }) => {
        ...
        const targets =
          window.__cadaraDebug?.getState()?.selectableTargets ?? [];
...
export class FeatureChainHarness {
  ...
  async addFilletFromFirstEdge() {
    ...
    await this.workbench.selectFirstReferenceMatching(
  ...
  async addPlaneFromTopPlane() {
    ...
    await this.workbench.page
      .getByRole("button", { name: /Top Plane/ })
      .first()
      .click();
    await expect
      .poll(() => this.workbench.currentEditorSelection(), { timeout: 10_000 })
      .toContain("construction_plane-xy");
