...
**Reason**: The migration is a breaking pre-alpha cleanup and must not retain deprecated validation or normalization branches that preserve old literal payload shapes.

**Migration**: Repo-owned fixtures, public sample documents, and tests must be canonicalized to the current authored-wrapper contract. Unsupported legacy literal payloads must fail validation with actionable messages rather than being partially normalized.
