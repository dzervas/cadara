...
export function appErrorFromValidationIssues(
  ...
  return createAppError({
    ...
    message:
      firstIssue?.message ??
      input.fallbackMessage ??
      `${input.operation} validation failed.`,
    context: [
...
export function appErrorFromModelingDiagnostic(
  ...
  return createAppError({
    ...
    context: [
      ...
      ...errorContext(
        "diagnosticDetail",
        diagnostic.detail ? diagnostic.detail.kind : null,
      ),
      ...(input.context ?? []),
    ],
    requestId: input.requestId,
...
export function appErrorFromModelingResult(input: {
  operation: string;
  fallbackMessage: string;
  ...
  if (primaryDiagnostic) {
    return appErrorFromModelingDiagnostic(primaryDiagnostic, {
      ...
      context: [
        ...
        ...errorContext(
          "actualRevisionId",
          input.revisionState?.actualRevisionId,
        ),
