...
export type GeometryAssetFormat =
  | "step"
  | "stl"
  | "3mf"
  | "baked-occ"
...
export type CadaraBrepFacetedTriangleInput = readonly [
  GeometryAssetPoint3,
  GeometryAssetPoint3,
  GeometryAssetPoint3,
];
...
export function normalizeGeometryAssetRecord(
...
export function normalizeGeometryAssetManifest(
  ...
  for (const record of manifest.records) {
    ...
    recordsById.set(
      normalized.assetId,
      normalizeGeometryAssetRecord({
        ...existing,
        ownerFeatureIds: [
          ...existing.ownerFeatureIds,
          ...normalized.ownerFeatureIds,
        ],
      }),
    );
  }
  ...
  return {
    schemaVersion: GEOMETRY_ASSET_MANIFEST_SCHEMA_VERSION,
    records: [...recordsById.values()].sort((left, right) =>
...
export function getEmbeddedGeometryAssetBytes(
  asset: GeometryAssetRecord,
): Uint8Array | null {
...
export function createCadaraFacetedBrepTopologyFromTriangles(
  ...
  const internEdge = (first: number, second: number) => {
    ...
    const parameterRange: GeometryAssetParameterRange = [
      0,
  ...
  for (const triangle of triangles) {
    const triangleVertices = triangle.map(internVertex) as [
      number,
      number,
      number,
    ];
    const points = triangleVertices.map((index) => vertices[index]!.point) as [
      GeometryAssetPoint3,
      GeometryAssetPoint3,
      GeometryAssetPoint3,
    ];
  ...
  if (faces.length === 0) {
    throw new Error(
      "Cadara B-rep topology requires at least one non-degenerate triangle.",
    );
  }
...
function createPointKey(point: GeometryAssetPoint3) {
  return point
    .map((coordinate) => normalizeCoordinate(coordinate).toFixed(9))
    .join(":");
}
...
function subtractPoints(
  left: GeometryAssetPoint3,
  right: GeometryAssetPoint3,
): GeometryAssetPoint3 {
  return [left[0] - right[0], left[1] - right[1], left[2] - right[2]];
}

function subtractPoints2(
  left: GeometryAssetPoint2,
  right: GeometryAssetPoint2,
): GeometryAssetPoint2 {
...
function projectPointToPlaneFrame(
  ...
  const yDirection = normalizeVector(
    crossPoints(frame.zDirection, frame.xDirection),
  );
...
function distanceBetweenPoints2(
  left: GeometryAssetPoint2,
  right: GeometryAssetPoint2,
) {
