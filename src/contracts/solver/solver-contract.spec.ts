...
test("src/contracts/solver/solver-contract.spec.ts", async () => {
  ...
  function createSolveRequest(
    projectedReferences: Awaited<
      ReturnType<SketchConstraintSolverAdapter["projectExternalReferences"]>
    >["projectedReferences"],
  ): SolveSketchRequest {
    return {
      contractVersion: CONTRACT_VERSION,
      ...
      projection.projectedReferences.every(
        (reference) =>
          ...
          reference.diagnostics.some(
            (diagnostic) =>
              diagnostic.code === "unsupported-model-reference-source",
          ),
      ),
