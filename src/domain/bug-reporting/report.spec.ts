...
test("src/domain/bug-reporting/report.spec.ts", async () => {
  function createStorage(value: string | null): StorageLike {
    return {
      getItem: (key) =>
        key === MODELING_OPERATION_HISTORY_STORAGE_KEY ? value : null,
      setItem: () => undefined,
  ...
  const operationHistory = createEmptyOperationHistory(
  ...
  expect(
    payloadResult.payload.editor.activeSketch?.entityCount === 0 &&
      payloadResult.payload.editor.activeFeatureEdit?.featureType ===
        "extrude" &&
      payloadResult.payload.editor.activeReferencePickerFieldId ===
        "profiles" &&
  ...
  assetDiagnosticSnapshot.document.diagnostics = [
    createGeometryAssetDiagnostic(
      ...
      asset.asset,
      "Referenced geometry asset bytes are corrupt.",
    ),
  ];
  ...
  const bounded = createBoundedMarkdownSections(
    [
      ...
      {
        id: "authoredDocument",
        summary: "Large section",
        value: { data: "x".repeat(200) },
      },
    ],
    {
      sectionByteLimit: 100,
      totalByteLimit: 500,
  ...
  const compactReport = JSON.parse(
    strFromU8(stateArchiveEntries["report/compact-report.json"]!),
  ) as typeof payloadResult.payload;
  const localStorageState = JSON.parse(
    strFromU8(stateArchiveEntries["state/local-storage.json"]!),
  ) as {
    entries: Record<string, string | null>;
  };
  const indexedDbState = JSON.parse(
