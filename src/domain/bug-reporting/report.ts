...
export const CADARA_GITHUB_BUG_REPORT_URL =
  "https://github.com/dzervas/cadara/issues/new";
...
const DOCUMENT_REPOSITORY_AUTOMERGE_URLS_STORAGE_KEY =
...
export interface BugReportEditorContext {
  ...
  activeCommand: Pick<
    NonNullable<EditorViewState["activeCommand"]>,
    "toolId" | "phase" | "commandSessionId"
  > | null;
  selectionFilter: {
    kind: string;
    label: string;
...
export interface BugReportPayloadResult {
  ...
  artifactSections: Partial<
    Record<"authoredDocument" | "operationHistory" | "runtimeTrace", unknown>
  >;
}
...
export function createBugReportPayload(
  ...
  const sectionByteLimit =
    options.sectionByteLimit ?? DEFAULT_SECTION_BYTE_LIMIT;
  const totalInlineByteLimit =
    options.totalInlineByteLimit ?? DEFAULT_TOTAL_INLINE_BYTE_LIMIT;
  const artifactSections: BugReportPayloadResult["artifactSections"] = {};
  const documentSummary = createDocumentSummary(input.snapshot);
  const authoredDocument = createAuthoredDocumentSection(
    input.snapshot,
    sectionByteLimit,
  );
  ...
  if (authoredDocument.status === "omitted-too-large" && input.snapshot) {
    artifactSections.authoredDocument = createAuthoredModelDocumentFromSnapshot(
      input.snapshot,
    );
  }
...
export function createBoundedMarkdownSections(
  ...
  const totalByteLimit =
    options.totalByteLimit ?? DEFAULT_TOTAL_INLINE_BYTE_LIMIT;
  const blocks: string[] = [];
  const omittedSectionIds: BugReportSectionId[] = [];
  let totalBytes = 0;
...
export function createBugReportIssueDraft(
  ...
  const attachmentFilename =
    artifactStatus.kind === "not-needed" ? null : artifactStatus.filename;
  const environmentMarkdown = createEnvironmentMarkdown(result.payload);
  const diagnostics = createBoundedMarkdownSections(
    [
      {
        id: "diagnostics",
        summary: "Diagnostics and errors",
        value: result.payload.diagnostics,
      },
    ],
    {
    ...
    [
      {
        ...
        value: result.payload.documentSummary,
      },
      {
        id: "authoredDocument",
        summary: "Authored document",
        value: result.payload.authoredDocument,
      },
      {
        id: "operationHistory",
        summary: "Operation history",
        value: result.payload.operationHistory,
      },
      {
        id: "runtimeTrace",
        summary: "Runtime trace",
        value: result.payload.runtimeTrace,
      },
      {
        id: "payloadStatus",
        summary: "Payload status",
        value: { ...result.payload.payloadStatus, artifact: artifactStatus },
      },
    ],
    {
      sectionByteLimit: options.sectionByteLimit,
  ...
  const debugPayloadMarkdown = [artifactNote, debugPayload.markdown]
    .filter(Boolean)
    .join("\n\n");
  const debugPayloadFallback = [
    artifactNote,
    createDownloadedDebugJsonPlaceholder(),
  ]
    .filter(Boolean)
    .join("\n\n");
  const fields = {
    ...
    [BUG_REPORT_FIELD_IDS.description]:
      "Generated from the Cadara workbench. Replace this with a concise bug description and reproduction steps.",
...
export function createGithubBugReportUrl(
  ...
  const fieldLimit =
    options.queryFieldByteLimit ?? DEFAULT_QUERY_FIELD_BYTE_LIMIT;
  const totalLimit =
    options.totalQueryByteLimit ?? DEFAULT_TOTAL_QUERY_BYTE_LIMIT;
  const fullFieldIds = new Set(options.fullQueryFieldIds ?? []);
  const fallbackValues = options.fallbackQueryFieldValues ?? {};
  let totalBytes = 0;

  url.searchParams.set(
    BUG_REPORT_FIELD_IDS.template,
  ...
  for (const [fieldId, value] of getOrderedIssueFields(fields)) {
    ...
    const nextBytes = getUtf8ByteLength(
      `${fieldId}=${encodeURIComponent(boundedValue)}`,
    );
    ...
    if (totalBytes + nextBytes > totalLimit) {
      const marker =
        "[Omitted from URL prefill because the bug-report query limit was reached.]";
      const markerBytes = getUtf8ByteLength(
        `${fieldId}=${encodeURIComponent(marker)}`,
      );
...
export async function createBugReportStateArchive(
  ...
  const entries: Record<string, Uint8Array> = {
    ...
    "state/local-storage.json": strToU8(
      stringifyJson(collectLocalStorageState(options.storage ?? null)),
    ),
    "state/indexeddb.json": strToU8(
      stringifyJson(
        await collectIndexedDbState(
  ...
  if (authoredDocument !== null) {
    entries["state/authored-document.json"] = strToU8(
      stringifyJson(authoredDocument),
    );
  }
  if (operationHistory !== null) {
    entries["state/operation-history.json"] = strToU8(
      stringifyJson(operationHistory),
    );
  }
...
export function downloadBugReportDebugArtifact(
  ...
  const blob = new Blob([createArtifactBlobPart(artifact.payload)], {
  ...
  try {
    ...
    return {
      ...
      reason:
        error instanceof Error
          ? error.message
          : "Authored document could not be serialized.",
    };
  }
}
...
function loadFullOperationHistory(
  storage: StorageLike | null,
): BugReportSection<ModelingOperationHistoryPayload> {
  if (!storage) {
    return {
  ...
  try {
    ...
    return {
      ...
      reason:
        error instanceof Error
          ? error.message
          : "Operation history could not be read from local storage.",
    };
  }
...
function createDiagnosticsContext(
  ...
  return {
    ...
    activeSketchDiagnostics:
      editorState.sketchSession?.projectionDiagnostics ?? [],
  };
}

function createEditorContext(
  editorState: BugReportPayloadInput["editorState"],
): BugReportEditorContext {
  ...
  return {
    ...
    selectionFilter: editorState.selectionFilter
      ? {
          ...
          requirementIds: editorState.selectionFilter.requirements.map(
            (requirement) => requirement.id,
          ),
        }
      : null,
    selectedTargets: editorState.selection.map((target) =>
    ...
    preview: editorState.preview
      ? {
          ...
          target: editorState.preview.target
            ? structuredClone(editorState.preview.target)
            : null,
        }
      : null,
...
function collectBrowserMetadata(
  navigatorLike = getGlobalNavigator(),
): BugReportBrowserMetadata {
...
function collectViewportMetadata(
  windowLike = getGlobalWindow(),
): BugReportViewportMetadata {
  return {
...
function collectRouteContext(
  locationLike = getGlobalWindow()?.location,
...
function collectWebglMetadata(
  documentLike = getGlobalDocument(),
): BugReportWebglMetadata {
  if (!documentLike) {
    return {
      status: "unavailable",
      vendor: null,
  ...
  try {
    ...
    return {
      ...
      reason:
        error instanceof Error
          ? error.message
          : "WebGL metadata could not be collected.",
    };
  }
}
...
function createArtifactStatus(
  result: BugReportPayloadResult,
): BugReportArtifactStatus {
  const artifact = createBugReportDebugArtifact(result);
  return artifact
    ? {
        kind: "unavailable",
        filename: artifact.filename,
        reason: "Debug artifact download was not attempted.",
      }
    : { kind: "not-needed" };
}
...
function isAllowedIssuePrefillField(fieldId: string) {
  return (
    fieldId === BUG_REPORT_FIELD_IDS.title ||
...
function collectLocalStorageState(storage: StorageLike | null) {
  if (!storage) {
    return {
      ...
      reason: "Browser localStorage is unavailable.",
    };
  }
...
async function collectIndexedDbState(
  ...
  const databases = (
    indexedDBFactory as IDBFactory & {
      databases?: () => Promise<
        readonly { name?: string | null; version?: number }[]
      >;
    }
  ).databases;
  ...
  try {
    ...
    const existingDatabaseNames = new Set(
      existingDatabases.map((database) => database.name).filter(isString),
    );
    const selectedDatabaseNames = databaseNames.filter((databaseName) =>
      existingDatabaseNames.has(databaseName),
    );

    return {
      status: "included" as const,
      databases: await Promise.all(
        selectedDatabaseNames.map((databaseName) =>
          readIndexedDbDatabase(indexedDBFactory, databaseName),
        ),
      ),
      missingDatabases: databaseNames.filter(
        (databaseName) => !existingDatabaseNames.has(databaseName),
      ),
    };
  } catch (error: unknown) {
...
async function readIndexedDbDatabase(
  ...
  try {
    ...
    return {
      ...
      objectStores: await Promise.all(
        storeNames.map((storeName) =>
          readIndexedDbObjectStore(database, storeName),
        ),
      ),
    };
  } finally {
...
function openIndexedDbDatabase(
  ...
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ??
          new Error(`IndexedDB database ${databaseName} could not be opened.`),
      );
    request.onblocked = () =>
      reject(new Error(`IndexedDB database ${databaseName} open was blocked.`));
  });
}

async function readIndexedDbObjectStore(
...
function getBugReportDownloadEnvironment(): BugReportDownloadEnvironment {
  if (!globalThis.document) {
    throw new Error(
      "Browser document is not available for bug-report download.",
    );
  }

  return {
    document:
      globalThis.document as unknown as BugReportDownloadEnvironment["document"],
    URL: globalThis.URL,
  };
}
