...
function makeTarget(): DurableRef {
  return {
    kind: "feature",
    featureId: "feature_export_target" as DurableRef extends {
      kind: "feature";
      featureId: infer T;
    }
      ? T
  ...
  return {
    ...
    getCompatibleProviders(target) {
      return providers.filter((provider) =>
        provider.targetKinds.includes(target.kind),
      );
    },
    getCompatibleFormats(target) {
      return this.getCompatibleProviders(target).map(
        (provider) => provider.formatId,
      );
    },
  };
}
