...
test("src/domain/editor/durable-sketch-authoring-operations.spec.ts", () => {
  ...
  expectTrue(
    rectangleOperation?.kind === "rectangle",
    "Rectangle creation should append one rectangle operation.",
  );
  expectTrue(
    rectangle.fullDefinition.authoringOperations?.length === 1,
  ...
  expectTrue(
    getSketchHistoryItems(rectangle.fullDefinition).length === 1,
  ...
  expectTrue(
    deleteOperation?.kind === "delete",
    "Deleting rectangle geometry should append a delete operation.",
  );
  expectTrue(
    deleted.definition.entityIds.length === 3,
  ...
  expectTrue(
    rolledBack.definition.entityIds.length === 4,
  ...
  expectTrue(
    rolledForward.definition.entityIds.length === 3,
  ...
  expectTrue(
    edited.fullDefinition.authoringOperations?.length === 1,
  ...
  expectTrue(
    JSON.stringify(firstSolve.solvedSnapshot.solvedEntities) ===
      JSON.stringify(secondSolve.solvedSnapshot.solvedEntities),
  ...
  expectTrue(
    liveEqualLengthConstraints.length === 1,
  ...
  expectTrue(
    constraintSession.fullDefinition.authoringOperations
      ...
      .slice(-3)
      .join(",") === "constraint,delete,constraint",
  ...
  expectTrue(
    recreated.definition.entityIds.length === 0,
  ...
  expectTrue(
    recreated.fullDefinition.authoringOperations
      ?.map((operation) => operation.kind)
      .join(",") === "rectangle,delete,rectangle",
