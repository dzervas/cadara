...
export function buildReferenceTargetPresentation(
...
export function beginSketchConstructionTool(
...
export function beginSketchReferenceTool(
  session: SketchSessionState,
): SketchSessionState {
...
export function buildSketchEditToolPresentation(
  ...
  const hoverLabel =
    state.hoverTarget?.kind === "sketchEntity" ? "Sketch entity" : null;
  ...
  const isReady =
    hasSelection && previewEntities.length > 0 && validationMessage === null;
  const promptText = isOffset
    ...
    : isTrim
      ...
      : hasSelection
        ? needsNumericValue
          ? `Set ${numericLabel.toLowerCase()}`
          : `Accept ${metadata.name.toLowerCase()}`
        : metadata.validationMessages.emptySelection;
  const validation = validationMessage
    ? [
        {
          id: `${state.toolId}-validation`,
          message: validationMessage,
        ...
        ? [
            {
              ...
              action: {
                type: "patch" as const,
                patch: { intent: "setOffsetDistance" },
              },
            },
            {
              id: "offset-side",
        ...
        ? [
            {
              ...
              action: {
                type: "patch" as const,
                patch: { intent: "setSketchEditValue" },
              },
            },
          ]
        : []),
    ...
    completionHints: [
      {
        ...
        text: isOffset
          ? hasSelection
            ? "Confirm to create offset geometry"
            : "Select connected lines or one curve"
          : hasSelection
            ? `Confirm ${metadata.previewLabel.toLowerCase()}`
            : metadata.validationMessages.emptySelection,
        ready: Boolean(isReady),
      },
    ],
    floatingInput:
      isOffset && hasSelection
        ? {
        ...
        : needsNumericValue && hasSelection
          ? {
              ...
              submitAction: {
                type: "patch",
                patch: { intent: "commitSketchEditOperator" },
              },
              cancelAction: {
                type: "patch",
                patch: { intent: "cancelSketchEditOperator" },
              },
            }
          : null,
...
export function getPreviewAnchor(
  previewEntities: readonly SketchDraftEntity[],
  ...
  const points =
    preview.kind === "spline" || preview.kind === "polyline"
      ? preview.points
      : [];
  const point = points[Math.floor(points.length / 2)];
  return point
    ? { kind: "sketchPoint", point, offset: { x: 18, y: -18 } }
    : undefined;
}

export function midpointSketchPoints(
  left: SketchPoint,
  right: SketchPoint,
): SketchPoint {
...
export function canAppendSketchEditToolActivationTarget(
  ...
  if (
    selectedTargets.some((selected) => primitiveRefEquals(selected, target))
  ) {
    return false;
  }
  ...
  if (
    ...
    selectedTargets.length >= metadata.selection.requiredCount
  ) {
    return false;
  }
  ...
  if (
    selectedTargets.some(
      (selected) => selected.kind === "projectedReferenceGeometry",
    ) ||
  ...
  const selectionKind = getSketchEditSelectionKindForEntity(
    session.definition,
    target.entityId,
  );
  return (
...
export function adoptCompatibleSketchEditToolTargets(
  ...
  for (const target of targets) {
    if (
      !canAppendSketchEditToolActivationTarget(
        session,
        toolId,
        adoptedTargets,
        target,
...
export function beginSketchEditTool(
  ...
  const adoptedTargets = adoptCompatibleSketchEditToolTargets(
    session,
    toolId,
...
export function updateSketchPointer(
  ...
  return sketchPointerPreviewStateEqual(session, nextSession)
    ? session
    : nextSession;
}
...
function sketchPointerPreviewStateEqual(
  ...
  return (
    ...
    sketchDraftEntitiesEqual(
      current.toolStagedEntities,
      next.toolStagedEntities,
    ) &&
    JSON.stringify(current.toolPresentation) ===
      JSON.stringify(next.toolPresentation)
  );
}
...
function dimensionAnnotationPlacementsEqual(
  ...
  right:
    | import("@/contracts/sketch/schema").DimensionAnnotationPlacement
    | null,
) {
  if (left === right) {
    return true;
...
export function selectSketchEditTarget(
  ...
  if (
    !session.definition.points.some((point) => point.pointId === target.pointId)
  ) {
    return session;
  }
  ...
  switch (target.kind) {
    ...
    case "vertex":
      return {
        ...
        projectionMode:
          target.kind === "face"
            ? "useExistingCoplanarGeometry"
            : "projectAlongPlaneNormal",
      };
    case "sketch":
    case "sketchEntity":
    case "sketchPoint":
...
export function selectSketchReferenceTarget(
  ...
  if (
    ...
    const message =
      "Reference geometry must come from model topology or another existing sketch.";
    return {
      ...session,
...
export function deleteSketchReferenceTarget(
  ...
  const removeFromDefinition = (
    definition: SketchDefinition,
  ): SketchDefinition => ({
    ...definition,
    referenceIds: definition.referenceIds.filter(
      (referenceId) => referenceId !== target.referenceId,
    ),
    references: definition.references.filter(
...
export function toggleConstructionTargetInDefinition(
  ...
  if (target.kind === "sketchEntity") {
    const selectedEntity = definition.entities.find(
    ...
    const points =
      selectedEntity.kind === "point"
        ? definition.points.map((point) =>
            point.pointId === selectedEntity.pointId
  ...
  const selectedPoint = definition.points.find(
...
export function pointsAlmostEqual(left: SketchPoint, right: SketchPoint) {
  return (
    Math.hypot(left[0] - right[0], left[1] - right[1]) <= SNAP_INFERENCE_EPSILON
  );
}

export function projectedKindForSource(
...
export function pointReadOnlyOperandFromSource(
  source: Extract<
    SketchSnapSourceRef,
    { kind: "projectedGeometry" | "sketchDatum" }
  >,
): ReadOnlySketchPointConstraintOperand {
...
export function getEntityEndpointAtPoint(
  ...
  for (const pointId of candidatePointIds) {
    const candidate = definition.points.find(
...
export function getEntityCenterPointId(
  definition: SketchDefinition,
  entityId: SketchEntityId,
): SketchPointId | null {
  const entity = definition.entities.find(
...
export function resolveReusableLocalSnapPointId(input: {
  ...
  for (const source of candidate.sources) {
    ...
    if (source.kind === "localEntity") {
      const pointId = getEntityEndpointAtPoint(
        previousDefinition,
        source.entityId,
        candidate.point,
      );
...
export function normalizeLinePatchEndpointReuse(input: {
  ...
  const lineEntity = input.patch.entities.find(
  ...
  return {
    ...input.patch,
    points: input.patch.points.filter(
      (point) => !replacements.has(point.pointId),
    ),
    entities: input.patch.entities.map((entity) => {
      ...
      return {
        ...entity,
        startPointId:
          replacements.get(entity.startPointId) ?? entity.startPointId,
...
export function getPatchLineEntityId(
  patch: SketchToolCommitContribution,
): SketchEntityId | null {
  return (
    patch.entities.find((entity) => entity.kind === "lineSegment")?.entityId ??
    null
  );
}
...
export function inferPointSnapConstraints(input: {
  ...
  const localCircleLikeEntityId = getPatchCircleLikeEntityAtCenter(
    input.patch,
    input.pointId,
  );
  ...
  input.candidate.sources.forEach((source, index) => {
    ...
    if (input.candidate.kind === "endpoint") {
      ...
      if (source.kind === "localEntity") {
        const endpointId = getEntityEndpointAtPoint(
      ...
      if (
        ...
        addConstraint(constraints, {
          constraintId: createId(
            source.kind === "sketchDatum"
              ? `inferred-coincident-datum-${index}`
              : `inferred-coincident-projected-${index}`,
          ),
    ...
    if (input.candidate.kind === "center") {
      if (source.kind === "localEntity") {
        ...
        const centerPointId = getEntityCenterPointId(
          input.previousDefinition,
          source.entityId,
        );
        if (centerPointId) {
      ...
      if (
        ...
        addConstraint(constraints, {
          constraintId: createId(
            source.kind === "sketchDatum"
              ? `inferred-coincident-datum-center-${index}`
              : `inferred-coincident-projected-center-${index}`,
          ),
          kind: "coincidentProjectedPoint",
          label:
            source.kind === "sketchDatum"
    ...
    if (input.candidate.kind === "midpoint") {
      ...
      if (
        ...
        addConstraint(constraints, {
          constraintId: createId(
            source.kind === "sketchDatum"
              ? `inferred-midpoint-datum-${index}`
              : `inferred-midpoint-projected-${index}`,
          ),
  ...
  candidate.sources.forEach((source, index) => {
    ...
    if (
      ...
      if (
        ...
        addConstraint(constraints, {
          constraintId: input.createConstraintId(
            source.kind === "sketchDatum"
              ? `inferred-perpendicular-datum-${index}`
              : `inferred-perpendicular-projected-${index}`,
          ),
        ...
        addConstraint(constraints, {
          constraintId: input.createConstraintId(
            source.kind === "sketchDatum"
              ? `inferred-foot-on-datum-${index}`
              : `inferred-foot-on-projected-${index}`,
          ),
          kind: "pointOnProjectedCurve",
          label:
            source.kind === "sketchDatum"
    ...
    if (
      ...
      if (source.kind === "localEntity") {
        ...
        addConstraint(constraints, {
          constraintId: input.createConstraintId(
            `inferred-tangent-point-on-${index}`,
          ),
          kind: "pointOnCurve",
      ...
      if (source.kind === "projectedGeometry") {
        addConstraint(constraints, {
          constraintId: input.createConstraintId(
            `inferred-tangent-projected-${index}`,
          ),
        ...
        addConstraint(constraints, {
          constraintId: input.createConstraintId(
            `inferred-tangent-point-projected-${index}`,
          ),
          kind: "pointOnProjectedCurve",
...
export function appendInferredSnapConstraints(input: {
  ...
  if (input.activeTool === "line") {
    const endPointId =
      patch.entities.find((entity) => entity.kind === "lineSegment")
        ?.endPointId ?? null;
    constraints.push(
      ...inferLineSnapConstraints({
        patch,
        candidate: input.endSnap,
        endPointId,
        sequence: input.sequence,
...
export function getConstraintDedupeKey(
  constraint: ConstraintDefinition,
): string {
  const readOnlyOperandKey = (operand: {
    kind: string;
    reference?: { referenceId: ReferenceId; geometryId: string };
...
function collinearTargetKey(operand: {
  kind: string;
  pointId?: string;
  entityId?: string;
}) {
...
export function acceptSketchDraw(
  ...
  if (result.state.validationMessage) {
    return {
      ...
      toolSettings: result.state.settings ?? session.toolSettings,
      validationMessage: result.state.validationMessage,
      toolPresentation: withSnapPresentation(
        result.presentation,
        snap.candidate,
      ),
      activeSnap: snap.candidate,
...
export function resolveSessionSnap(
  ...
  const snap = resolveSketchSnap({
    ...
    activeTool: session.activeTool,
    activeAnchor:
      session.status === "drawing" ? session.pointerDownPoint : null,
...
export function getSnapConstraintPreviewOverlays(
  candidate: SketchSnapCandidate,
): SketchToolOverlayDescriptor[] {
...
export function getToolRuntimeState(
  session: SketchSessionState,
): import("@/core/sketch-tools/definition").SketchToolRuntimeState {
  return {
...
export function getSketchSessionPreviewLabel(
  session: SketchSessionState,
): string {
  if (session.activeAnnotationEdit) {
    return (
      session.toolPresentation?.prompts[0]?.text ?? "Edit annotation value"
    );
  }
  ...
  if (session.constraintAuthoring) {
    const definition = getSketchConstraintDefinition(
      session.constraintAuthoring.toolId,
    );
    const valueSpec =
      definition.getValueSpec?.(session.constraintAuthoring.selectedTargets) ??
  ...
  const primaryPrompt =
    session.toolPresentation?.validation?.[0]?.message ??
  ...
  if (session.projectionDiagnostics.length > 0) {
    return (
      session.projectionDiagnostics[0]?.message ??
      "Sketch reference projection has diagnostics"
    );
  }

  const regionDiagnostic = getSketchSessionRegionDiagnostics(session).find(
...
export function getSketchToolPresentation(
  session: SketchSessionState,
): SketchToolPresentationSchema | null {
  if (session.activeSpecialMode) {
    return null;
  }
  ...
  if (session.activeStyleFocus) {
    return buildSketchStylePresentation(
      session.activeStyleFocus,
      getSketchStyleTargetDefinition(session, session.activeStyleFocus.target)
        ?.style,
    );
  }
  ...
  if (!isDrawingSketchTool(session.activeTool)) {
    if (!session.activeEditTarget) {
      const overlays = buildCommittedDimensionOverlays(
        session,
        session.definition,
      );
  ...
  return (
    ...
    getSketchToolDefinition(session.activeTool).getPresentation(
...
export function patchSketchDrawingToolValue(
  ...
  return {
    ...
    toolStagedEntities: withConstructionFlag(
      toolDefinition.getStagedEntities(runtimeState),
      session.constructionModifierActive,
    ),
