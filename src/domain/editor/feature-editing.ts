...
function featureDraftChanged(
  ...
  return (
    JSON.stringify(previousSession.draft) !== JSON.stringify(nextSession.draft)
  );
}
...
export function createPreviewMissingInputsDiagnostics(
  ...
  const definition = getFeatureAuthoringDefinition(session.featureType);
  return definition.getMissingInputsDiagnostics({
    draft: session.draft,
    phase: "preview",
  });
}
