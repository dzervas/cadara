...
export function createInitialWorkbenchTabsState(
  seed: WorkbenchTab,
): WorkbenchTabsState {
  ...
  switch (action.type) {
    case "open": {
      const existingIndex = state.tabs.findIndex(
        (tab) => tab.documentId === action.tab.documentId,
      );
      if (existingIndex >= 0) {
        ...
        return {
          tabs: nextTabs,
          activeDocumentId: action.activate
            ? action.tab.documentId
    ...
    case "close": {
      ...
      const closedIndex = state.tabs.findIndex(
        (tab) => tab.documentId === action.documentId,
      );
      if (closedIndex < 0) {
        return state;
      }
      ...
      return {
        tabs: nextTabs,
        activeDocumentId: nextTabs[successorIndex].documentId,
      };
    }
    ...
    case "reorder": {
      ...
      const clampedTo = Math.max(
        0,
        Math.min(state.tabs.length - 1, action.toIndex),
      );
...
export function reconcileWorkbenchTabsForActiveDocument(
  ...
  const hasActive = persisted.tabs.some(
    (tab) => tab.documentId === active.documentId,
  );
  if (hasActive) {
