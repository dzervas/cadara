...
function applyCursorToMockSnapshot(snapshot: WorkspaceSnapshot) {
  ...
  snapshot.document.render.records = snapshot.document.render.records.filter(
    (record) =>
      (record.ownerFeatureId === null ||
        appliedFeatureIds.has(record.ownerFeatureId)) &&
      isRenderTargetAppliedForMockCursor(
  ...
  snapshot.presentation.entities = snapshot.presentation.entities.filter(
    (entity) =>
      (entity.ownerFeatureId === null ||
        appliedFeatureIds.has(entity.ownerFeatureId)) &&
      (entity.ownerSketchId === null ||
        appliedSketchIds.has(entity.ownerSketchId)),
  );
  snapshot.presentation.objects = snapshot.presentation.objects.filter(
    (object) =>
      (object.ownerFeatureId === null ||
        appliedFeatureIds.has(object.ownerFeatureId)) &&
      (object.ownerSketchId === null ||
        appliedSketchIds.has(object.ownerSketchId)),
  );
  snapshot.presentation.featureTree = snapshot.presentation.featureTree.filter(
    (node) =>
      node.kind === "plane" ||
      ((node.ownerFeatureId === null ||
        appliedFeatureIds.has(node.ownerFeatureId)) &&
        (node.ownerSketchId === null ||
          appliedSketchIds.has(node.ownerSketchId))),
  );
...
function assertSupportedModelingRequest(request: {
...
function isSketchRenameOnlyRequest(
  ...
  if (
    !existing ||
    !request.sketchId ||
    existing.label === request.sketchLabel
  ) {
    return false;
  }
  ...
  return (
    JSON.stringify(existing.plane) === JSON.stringify(request.plane) &&
    JSON.stringify(existing.sketch.definition) ===
...
function getConsumedEntityKeysForTarget(target: DurableRef) {
  ...
  if (target.kind === "region") {
    keys.push(
      getPrimitiveRefKey({ kind: "sketch", sketchId: target.sketchId }),
    );
  }
...
function createMissingDocumentHistoryItemDiagnostic(
  item: ReorderDocumentHistoryRequest["item"],
) {
  return {
...
function createDocumentHistoryDependencyOrderDiagnostic(
  violation: ReturnType<
    typeof findDocumentHistoryOrderDependencyViolations
  >[number],
) {
  return {
...
function reorderFeaturesByDocumentHistory<
  TFeature extends { featureId: FeatureId },
>(
...
function getDeleteTargetHistoryEntry(
  target: DurableRef,
...
function createMockExportCapabilities(
  snapshot: WorkspaceSnapshot,
): ExportCapabilities {
  return {
    mesh: {
      tessellate: (
        ...
        if (target.kind !== "body") {
          return {
            ...
            severity: "error",
            message:
              "Only solid body rows can be exported to geometry formats.",
            target,
          };
        }
    ...
    sketchVector: {
      resolveSketchVectorModel: (target: DurableRef) => {
        const result = buildSketchVectorExportModelFromSnapshot(
          snapshot,
          target,
        );
...
function createInvalidFeatureDiagnostic(
  ...
  target: NonNullable<
    ReturnType<typeof getFeatureDefinitionChangedTargets>[number]
  >,
  message: string,
) {
  return {
...
function hasRegionTarget(
  snapshot: WorkspaceSnapshot,
  target: Extract<
    FeatureDefinition,
    { kind: "extrude" }
  >["parameters"]["profiles"][number],
) {
  return (
    target.kind === "region" &&
    snapshot.document.sketches.some(
      (sketch) =>
        sketch.sketchId === target.sketchId &&
...
function hasFaceTarget(
  snapshot: WorkspaceSnapshot,
  bodyId: string,
  faceId: FaceId,
) {
  return snapshot.document.bodies.some(
    (body) => body.bodyId === bodyId && body.topology.faceIds.includes(faceId),
  );
}
...
function hasEdgeTarget(
  ...
  return snapshot.document.bodies.some(
    (body) =>
      body.bodyId === bodyId &&
      body.topology.edgeIds.includes(
        edgeId as (typeof body.topology.edgeIds)[number],
      ),
  );
}

function hasConstructionTarget(
  snapshot: WorkspaceSnapshot,
  constructionId: string,
) {
  return snapshot.document.constructions.some(
    (construction) => construction.constructionId === constructionId,
  );
}

function hasSketchEntityTarget(
  snapshot: WorkspaceSnapshot,
  sketchId: SketchId,
  ...
  return snapshot.document.sketches.some(
    (sketch) =>
      ...
      sketch.sketch.definition.entities.some(
...
function validateFeatureDefinitionAgainstSnapshot(
  ...
  switch (definition.kind) {
    case "extrude": {
      ...
      if (
        firstProfile.kind === "face" &&
        !isSupportedPlanarFace(firstProfile.faceId)
      ) {
        return {
          accepted: false as const,
          reasonCode: "mock-invalid-extrude",
    ...
    case "fillet": {
      ...
      if (
        definition.parameters.edgeTargets.some(
          (target) => !hasEdgeTarget(snapshot, target.bodyId, target.edgeId),
        )
      ) {
        return {
          accepted: false as const,
      ...
      if (
        (definition.parameters.reference.target.kind === "construction" &&
          !hasConstructionTarget(
            snapshot,
            definition.parameters.reference.target.constructionId,
          )) ||
        (definition.parameters.reference.target.kind === "face" &&
          !hasFaceTarget(
            snapshot,
            definition.parameters.reference.target.bodyId,
            definition.parameters.reference.target.faceId,
          ))
      ) {
        return {
          accepted: false as const,
      ...
      return {
        ...
        diagnostics: [
          createUnsupportedFeatureDiagnostic(
            definition,
            "Mock kernel does not implement revolve creation yet.",
          ),
        ],
      };
    }
    case "shell":
      return {
        ...
        diagnostics: [
          createUnsupportedFeatureDiagnostic(
            definition,
            "Mock kernel does not implement shell creation yet.",
          ),
        ],
      };
    case "sweep": {
      ...
      const pathTargets =
        getAdvancedParticipant(definition, "path")?.targets ?? [];
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Sweep requires at least one profile participant and exactly one path participant.",
            ),
          ],
        };
      }
      ...
      if (
        profileTargets.some(
          (target) => target.kind !== "region" && target.kind !== "face",
        )
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Sweep profile participants must be region or face targets.",
            ),
          ],
        };
      }
      ...
      if (firstPath.kind !== "edge" && firstPath.kind !== "sketchEntity") {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Sweep path participant must be an edge or sketch entity target.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Sweep profile and path participants must resolve to live durable targets.",
            ),
          ],
        };
      }
      ...
      if (guideCurveTargets.length > 0) {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock sweep does not implement guide-curve participants yet.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        if (
          ...
          targetBodyTargets.some((target) => target.kind !== "body")
        ) {
          return {
            accepted: false as const,
            reasonCode: "mock-invalid-sweep",
            diagnostics: [
              createUnsupportedFeatureDiagnostic(
                definition,
                "Sweep boolean operation intents require explicit targetBody participants.",
              ),
            ],
          };
        }
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock sweep does not implement boolean composition yet.",
            ),
          ],
        };
      }
    ...
    case "loft": {
      ...
      if (profileTargets.length < 2) {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft requires at least two ordered profile participants.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft profile participants must be region or face targets.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft profile participants must resolve to live durable targets.",
            ),
          ],
        };
      }
      ...
      if (
        pathTargets.length > 1 ||
        pathTargets.some(
          (target) => target.kind !== "edge" && target.kind !== "sketchEntity",
        )
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft path participant must be one edge or sketch entity target.",
            ),
          ],
        };
      }
      ...
      if (
        pathTargets.some(
          (target) =>
            (target.kind === "edge" &&
              !hasEdgeTarget(snapshot, target.bodyId, target.edgeId)) ||
            (target.kind === "sketchEntity" &&
              !hasSketchEntityTarget(
                snapshot,
                target.sketchId,
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft path participant must resolve to a live durable target.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft guide-curve participants must be edge or sketch entity targets.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Loft guide-curve participants must resolve to live durable targets.",
            ),
          ],
        };
      }
      ...
      if (pathTargets.length > 0 && guideCurveTargets.length > 0) {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock loft does not implement combined path and guide-curve participants yet.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        if (
          ...
          return {
            ...
            diagnostics: [
              createUnsupportedFeatureDiagnostic(
                definition,
                "Loft boolean operation intents require explicit targetBody participants.",
              ),
            ],
          };
        }
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock loft does not implement boolean composition yet.",
            ),
          ],
        };
      }
    ...
    case "chamfer": {
      const edgeTargets =
        getAdvancedParticipant(definition, "edge")?.targets ?? [];
      const distance = definition.parameters.options?.distance;

      if (
        edgeTargets.length === 0 ||
        edgeTargets.some((target) => target.kind !== "edge")
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Chamfer requires at least one durable edge participant.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Chamfer requires a positive constant distance option.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Chamfer edge participants must resolve to live durable edges.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock chamfer does not implement operation intents yet.",
            ),
          ],
        };
      }
    ...
    case "thicken": {
      const faceTargets =
        getAdvancedParticipant(definition, "face")?.targets ?? [];
      const targetBodyTargets =
        getAdvancedParticipant(definition, "targetBody")?.targets ?? [];
      ...
      if (
        ...
        faceTargets.some((target) => target.kind !== "face")
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Thicken requires at least one durable face participant.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Thicken requires a positive thickness option.",
            ),
          ],
        };
      }
      ...
      if (side !== undefined && side !== "oneSide" && side !== "symmetric") {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Thicken side must be oneSide or symmetric.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Thicken face participants must resolve to live durable faces.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        if (
          ...
          return {
            ...
            diagnostics: [
              createUnsupportedFeatureDiagnostic(
                definition,
                "Thicken boolean operation intents require explicit targetBody participants.",
              ),
            ],
          };
        }
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock thicken does not implement boolean composition yet.",
            ),
          ],
        };
      }
    ...
    case "combine": {
      ...
      if (intent !== "add" && intent !== "subtract" && intent !== "intersect") {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Combine requires add, subtract, or intersect operation intent.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Combine requires at least one explicit targetBody participant.",
            ),
          ],
        };
      }
      ...
      if (
        toolBodyTargets.length === 0 ||
        toolBodyTargets.some((target) => target.kind !== "body")
      ) {
        return {
          accepted: false as const,
          reasonCode: "mock-invalid-combine",
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Combine requires at least one explicit toolBody participant.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Combine body participants must resolve to live durable bodies.",
            ),
          ],
        };
      }

      const targetBodyIds = new Set(
        targetBodyTargets.map((target) =>
          target.kind === "body" ? target.bodyId : null,
        ),
      );
      if (
        toolBodyTargets.some(
          (target) =>
            target.kind === "body" && targetBodyIds.has(target.bodyId),
        )
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Combine targetBody and toolBody participants must be distinct.",
            ),
          ],
        };
      }
    ...
    case "split": {
      ...
      const toolBodyTargets =
        getAdvancedParticipant(definition, "toolBody")?.targets ?? [];
      const planeTargets =
        getAdvancedParticipant(definition, "plane")?.targets ?? [];

      if (
        targetBodyTargets.length !== 1 ||
        targetBodyTargets.some((target) => target.kind !== "body")
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Split requires exactly one explicit targetBody participant.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Split requires exactly one explicit toolBody participant in the initial implementation.",
            ),
          ],
        };
      }
      ...
      if (planeTargets.length > 0) {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock split does not implement plane split tools yet.",
            ),
          ],
        };
      }
      ...
      if (targetBody?.kind !== "body" || toolBody?.kind !== "body") {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Split targetBody and toolBody participants must resolve to live durable bodies.",
            ),
          ],
        };
      }
      ...
      if (
        !hasBodyTarget(snapshot, targetBody.bodyId) ||
        !hasBodyTarget(snapshot, toolBody.bodyId)
      ) {
        return {
          accepted: false as const,
          reasonCode: "mock-invalid-split",
    ...
    case "deleteSolid": {
      ...
      if (
        ...
        bodyTargets.some((target) => target.kind !== "body")
      ) {
        return {
          accepted: false as const,
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Delete-solid requires one or more explicit body participants.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Delete-solid body participants must resolve to live durable bodies.",
            ),
          ],
        };
      }
    ...
    case "mirror": {
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mirror requires one or more explicit body participants.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mirror requires exactly one planar face or construction plane participant.",
            ),
          ],
        };
      }
      ...
      if (copy !== true) {
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mock mirror currently supports copy=true only.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mirror body participants must resolve to live durable bodies.",
            ),
          ],
        };
      }
      ...
      if (
        (planeTarget?.kind === "construction" &&
          !hasConstructionTarget(snapshot, planeTarget.constructionId)) ||
        (planeTarget?.kind === "face" &&
          !hasFaceTarget(snapshot, planeTarget.bodyId, planeTarget.faceId))
      ) {
        return {
          accepted: false as const,
          reasonCode: "mock-invalid-mirror",
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Mirror planar references must resolve to live construction planes or planar faces.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Transform requires one or more explicit body participants.",
            ),
          ],
        };
      }
      ...
      if (
        ...
        return {
          ...
          diagnostics: [
            createUnsupportedFeatureDiagnostic(
              definition,
              "Transform body participants must resolve to live durable bodies.",
            ),
          ],
        };
      }
    ...
    default:
      return {
        ...
        diagnostics: [
          createUnsupportedFeatureDiagnostic(
            definition,
            `Mock kernel does not implement ${definition.kind} advanced solid features yet.`,
          ),
        ],
      };
  }
...
function buildPreviewRenderables(
  definition: FeatureDefinition,
  snapshot: WorkspaceSnapshot,
) {
...
function getCommitSolverCorrelation(
  ...
  if (!request.solverCorrelation) {
    throw new Error(
      "Sketch commit request must include explicit solverCorrelation request IDs.",
    );
  }
    ...
    target:
      diagnostic.target?.kind === "entity"
        ? {
            ...
            sketchId,
            entityId: diagnostic.target.entityId,
          }
        : diagnostic.target?.kind === "point"
          ? {
              kind: "sketchPoint" as const,
              sketchId,
              pointId: diagnostic.target.pointId,
            }
          : diagnostic.target?.kind === "region"
            ? {
                kind: "region" as const,
                sketchId,
                regionId: diagnostic.target.regionId,
              }
            : null,
    detail: null,
  };
...
function createSketchReferenceFrame(input: {
  planeTarget:
    | SketchPlaneDefinition["support"]
    | SketchRecord["definition"]["references"][number]["source"];
  planeKey: "xy" | "yz" | "xz";
}) {
  const byPlaneKey = {
    xy: {
    ...
    "xy" | "yz" | "xz",
    typeof DEFAULT_MOCK_SKETCH_PLANE_FRAME
  >;
...
async function buildSnapshot(
  ...
  if (!primaryRegion) {
    throw new Error(
      "Mock snapshot bootstrap expected a solver-derived primary region.",
    );
  }
  ...
  const entities = [
    ...
    entity({
      ...
      relatedTargets: [
        ...
        },
        {
          kind: "vertex",
          bodyId: "body_part-1",
          vertexId: "vertex_front-left",
        },
        {
          kind: "vertex",
          bodyId: "body_part-1",
          vertexId: "vertex_back-right",
        },
    ...
    entity({
      ...
      target: {
        kind: "face",
        bodyId: "body_part-1",
        faceId: "face_side-front",
      },
      relatedTargets: [
        { kind: "body", bodyId: "body_part-1" },
        { kind: "feature", featureId: "feature_fillet-1" },
      ],
      consumedByFeatureIds: [],
    }),
    entity({
  ...
  document.cursor = createTailDocumentCursor(
    createDocumentHistoryItems({
...
function entity(
  ...
  const selectionSemantics =
    input.selectionSemantics ?? getSelectionSemanticsForTarget(input.target);
  return {
    ownerDocumentId: DOCUMENT_ID,
    ownerRevisionId: REVISION_ID,
    ...input,
    contributingFeatureIds:
      input.contributingFeatureIds ??
      (input.ownerFeatureId ? [input.ownerFeatureId] : []),
    selectionSemantics,
  };
}

function updateSketchRecordRevision(
  sketch: SketchRecord,
  revisionId: RevisionId,
) {
  sketch.ownerRevisionId = revisionId;
  sketch.solvedSnapshot.constraintStatuses = [
    ...sketch.solvedSnapshot.constraintStatuses,
  ];
  sketch.solvedSnapshot.dimensionStatuses = [
    ...sketch.solvedSnapshot.dimensionStatuses,
  ];
  sketch.solvedSnapshot.solvedEntities = [
    ...sketch.solvedSnapshot.solvedEntities,
  ];
  sketch.solvedSnapshot.solvedPoints = [...sketch.solvedSnapshot.solvedPoints];
  sketch.regions = sketch.regions.map((region) => ({
    ...region,
    ownerRevisionId: revisionId,
  }));
}

function stampSnapshotRevision(
  snapshot: WorkspaceSnapshot,
  revisionId: RevisionId,
) {
...
function getSelectionSemanticsForTarget(
  target: SnapshotEntityRecord["target"],
): SnapshotEntityRecord["selectionSemantics"] {
  switch (target.kind) {
    case "body":
      return ["body"];
    case "face":
      return target.faceId === "face_top"
        ? ["face", "planarFace", "planarReference"]
        : ["face"];
    case "edge":
      return ["edge"];
    case "vertex":
...
function updateFeatureEntityRelationship(
  ...
  const changedKeys = new Set(
    changedTargets.flatMap((target) => getConsumedEntityKeysForTarget(target)),
  );

  for (const entityRecord of snapshot.presentation.entities) {
    const entityKey = getPrimitiveRefKey(entityRecord.target);
    if (
      changedKeys.has(entityKey) &&
      !entityRecord.consumedByFeatureIds.includes(featureId)
    ) {
      entityRecord.consumedByFeatureIds = [
        ...entityRecord.consumedByFeatureIds,
        featureId,
      ];
    }
...
function rebuildFeatureTree(snapshot: WorkspaceSnapshot) {
  const historyOrder = snapshot.presentation.documentHistory?.length
    ...
    : undefined;
  const sketchNodes = new Map(
    snapshot.document.sketches.map((sketch, index) => [
      sketch.sketchId,
      {
        id: (index === 0
          ? "feature_tree_node_sketch_1"
          : `feature_tree_node_sketch_${index + 1}`) as FeatureTreeNodeId,
        label: sketch.label,
...
function rebuildObjectTree(snapshot: WorkspaceSnapshot) {
  const nonSketchObjects = snapshot.presentation.objects.filter(
  ...
  snapshot.presentation.objects = [
    ...
    ...snapshot.document.sketches.map((sketch, index) => ({
      id: (index === 0
        ? "object_tree_node_sketch_1"
        : `object_tree_node_sketch_${index + 1}`) as ObjectTreeNodeId,
      label: sketch.label,
      description: `Sketch on ${(sketch.plane.key ?? "xy").toUpperCase()} plane`,
      kind: "sketch" as const,
      target: { kind: "sketch" as const, sketchId: sketch.sketchId },
...
function getMockCombineBodyIds(
  ...
  return (getAdvancedParticipant(definition, role)?.targets ?? []).flatMap(
...
function applyMockCombineSnapshotMutation(
  ...
  snapshot.document.bodies = snapshot.document.bodies
    ...
    .map((body) =>
      targetBodyIdSet.has(body.bodyId)
        ? {
            ...body,
            ownerFeatureId: featureId,
  ...
  snapshot.presentation.objects = snapshot.presentation.objects
    .filter(
      (item) =>
        item.target.kind !== "body" || !toolBodyIdSet.has(item.target.bodyId),
    )
    .map((item) =>
      item.target.kind === "body" && targetBodyIdSet.has(item.target.bodyId)
        ? {
            ...item,
            ownerFeatureId: featureId,
  ...
  snapshot.document.render.records = snapshot.document.render.records
    .filter(
      (record) =>
        record.ownerBodyId === null || !toolBodyIdSet.has(record.ownerBodyId),
    )
    .map((record) =>
      record.ownerBodyId !== null && targetBodyIdSet.has(record.ownerBodyId)
        ? {
            ...record,
            ownerFeatureId: featureId,
  ...
  snapshot.presentation.entities = snapshot.presentation.entities
    .filter(
      (entry) =>
        entry.ownerBodyId === null || !toolBodyIdSet.has(entry.ownerBodyId),
    )
    .map((entry) =>
      entry.ownerBodyId !== null && targetBodyIdSet.has(entry.ownerBodyId)
        ? {
            ...entry,
            ownerFeatureId: featureId,
...
function allocateDocumentVariableId(
  snapshot: WorkspaceSnapshot,
): DocumentVariableId {
  let nextOrdinal = 1;
...
export class MockKernelAdapter implements ModelingKernelAdapter {
  ...
  async exportAuthoredModelDocument(
    ...
    if (snapshot.document.documentId !== documentId) {
      throw new Error(
        `Mock authored export requested document ${documentId}, but active document is ${snapshot.document.documentId}.`,
      );
    }
  ...
  async restoreAuthoredModelDocument(
    ...
    const featureTargets = new Map(
      snapshot.document.features.map((feature) => [
        feature.featureId,
        feature.producedTargets,
      ]),
    );
    const featureById = new Map(
      document.features.map((feature) => [feature.featureId, feature]),
    );
    const orderedFeatures = document.featureOrder
      ...
      .filter(
        (
          feature,
        ): feature is RepositoryAuthoredModelDocument["features"][number] =>
          Boolean(feature),
      );
    ...
    snapshot.document.features = orderedFeatures.map((feature) => ({
      ...
      producedTargets: feature.suppressed
        ? []
        : structuredClone(featureTargets.get(feature.featureId) ?? []),
    }));
    ...
    for (const label of document.bodyLabels) {
      const body = snapshot.document.bodies.find(
        (entry) => entry.bodyId === label.bodyId,
      );
      if (body) {
        body.label = label.label;
      }
      const object = snapshot.presentation.objects.find(
        (entry) =>
          entry.target.kind === "body" && entry.target.bodyId === label.bodyId,
      );
      if (object) {
  ...
  private async mutateSnapshot<TResponse>(
    mutate: (
      snapshot: WorkspaceSnapshot,
      nextRevisionId: RevisionId,
    ) => TResponse,
    ...
    const nextRevisionId =
      `rev_${String(this.revisionSequence).padStart(4, "0")}` as RevisionId;
  ...
  async getDocumentSnapshot(
    _request: GetDocumentSnapshotRequest,
  ): Promise<GetDocumentSnapshotResponse> {
    assertSupportedModelingRequest(_request);

    return {
      contractVersion: CONTRACT_VERSION,
      snapshot: applyCursorToMockSnapshot(
        structuredClone(await this.getSnapshot()),
      ),
    };
  }
  ...
  async createFeature(
    ...
    if (hasRevisionConflict(request.baseRevisionId, this.currentRevisionId)) {
      const diagnostics = [
        createRevisionConflictDiagnostic(
    ...
    if (!resolvedDefinition.ok) {
      return {
        ...
        revisionState: {
          ...
          reasonCode:
            resolvedDefinition.diagnostics[0]?.code ??
            "feature-value-expression-invalid",
        },
        rebuildResult: createRebuildResult({
          kind: "skipped",
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      mutableSnapshot.presentation.entities.push(
        entity({
          ownerFeatureId: featureId,
          ownerSketchId: null,
          ownerBodyId: null,
      ...
      applyMockCombineSnapshotMutation(
        mutableSnapshot,
        featureId,
        resolvedDefinition.definition,
      );
      ...
      const diagnostics = [
        {
          ...
          message:
            "Mock kernel committed the feature create request into the current revision.",
  ...
  async commitSketch(
    ...
    const snapshot = await this.getSnapshot();
    const sketchId =
      request.sketchId ?? allocateMockSketchId(snapshot.document.sketches);
    ...
    if (isSketchRenameOnlyRequest(request, existingSketch)) {
      return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
        ...
        for (const entityRecord of mutableSnapshot.presentation.entities) {
          if (
            entityRecord.target.kind === "sketch" &&
            entityRecord.target.sketchId === request.sketchId
          ) {
        ...
        const diagnostics = [
          {
            ...
            severity: "info" as const,
            message:
              "Mock kernel renamed the sketch without rebuilding unchanged geometry.",
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      const changedTargets = [
        ...
        ...normalizedDefinition.points.map((point) => ({
          ...point.target,
          sketchId,
        })),
      ];

      const existingIndex = mutableSnapshot.document.sketches.findIndex(
      ...
      mutableSnapshot.presentation.entities =
        mutableSnapshot.presentation.entities.filter(
          (entry) => entry.ownerSketchId !== sketchId,
        );
      mutableSnapshot.presentation.entities.push(
        entity({
          ownerFeatureId: null,
      ...
      mutableSnapshot.presentation.entities.push(
        ...sketchRecord.regions.map((region, index) =>
          entity({
            ownerFeatureId: null,
            ownerSketchId: sketchId,
            ownerBodyId: null,
      ...
      mutableSnapshot.presentation.entities.push(
        ...normalizedDefinition.entities.map((entry, index) =>
          entity({
            ownerFeatureId: null,
            ownerSketchId: sketchId,
            ownerBodyId: null,
      ...
      mutableSnapshot.presentation.entities.push(
        ...normalizedDefinition.points.map((point, index) =>
          entity({
            ownerFeatureId: null,
            ownerSketchId: sketchId,
            ownerBodyId: null,
      ...
      const diagnostics = [
        ...commitDiagnostics,
        {
          ...
          severity: "info" as const,
          message:
            "Mock kernel committed the sketch definition into the current revision.",
          target: createSketchTarget(sketchId),
          detail: null,
        },
      ];
  ...
  async updateFeature(
    ...
    const existingFeatureIndex = snapshot.document.features.findIndex(
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      const featureEntity = mutableSnapshot.presentation.entities.find(
        (entry) =>
          entry.target.kind === "feature" &&
          entry.target.featureId === request.featureId,
      );
      if (featureEntity) {
      ...
      const diagnostics: UpdateFeatureResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the feature update into the current revision.",
          target: mutableFeature.producedTargets[0] ?? {
            kind: "feature",
            featureId: request.featureId,
          },
          detail: null,
  ...
  async setFeatureSuppression(
    ...
    const existingFeature = snapshot.document.features.find(
      (feature) => feature.featureId === request.featureId,
    );
    if (!existingFeature) {
      const diagnostics = [createMissingFeatureDiagnostic(request.featureId)];
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      mutableSnapshot.presentation.documentHistory = createDocumentHistoryItems(
        {
          ...
          features: mutableSnapshot.document.features,
          sketches: mutableSnapshot.document.sketches,
          historyOrder: createDocumentHistoryOrder(
            mutableSnapshot.presentation.documentHistory,
          ),
        },
      );
      ...
      const diagnostics: SetFeatureSuppressionResponse["diagnostics"] = [
        {
          code: request.suppressed
            ? "mock-suppress-feature"
            : "mock-unsuppress-feature",
          severity: "info",
  ...
  async deleteFeature(
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      if (
        ...
        mutableSnapshot.document.cursor = createTailDocumentCursor(
          mutableSnapshot.presentation.documentHistory,
        );
      }
      mutableSnapshot.presentation.entities =
      ...
      for (const entityRecord of mutableSnapshot.presentation.entities) {
        entityRecord.consumedByFeatureIds =
          entityRecord.consumedByFeatureIds.filter(
            (featureId) => featureId !== request.featureId,
          );
      }
      const diagnostics: DeleteFeatureResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the feature deletion into the current revision.",
          target: { kind: "feature", featureId: request.featureId },
          detail: null,
        },
      ];
  ...
  async deleteTarget(
    ...
    if (request.target.kind === "feature" || request.target.kind === "sketch") {
      ...
      const historyOrder = createDocumentHistoryOrder(
        snapshot.presentation.documentHistory,
      );
      ...
      return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
        ...
        if (target.kind === "feature") {
          mutableSnapshot.document.features =
            mutableSnapshot.document.features.filter(
              (feature) => feature.featureId !== target.featureId,
            );
          mutableSnapshot.presentation.entities =
            mutableSnapshot.presentation.entities.filter(
              (entry) =>
                !(
                  entry.target.kind === "feature" &&
                  entry.target.featureId === target.featureId
                ),
            );
          for (const entityRecord of mutableSnapshot.presentation.entities) {
            entityRecord.consumedByFeatureIds =
          ...
          mutableSnapshot.document.sketches =
            mutableSnapshot.document.sketches.filter(
              (sketch) => sketch.sketchId !== target.sketchId,
            );
          mutableSnapshot.presentation.entities =
            mutableSnapshot.presentation.entities.filter(
              (entry) => entry.ownerSketchId !== target.sketchId,
            );
        }

        mutableSnapshot.document.entities =
          mutableSnapshot.presentation.entities;
        mutableSnapshot.presentation.documentHistory =
          createDocumentHistoryItems({
            featureTree: mutableSnapshot.presentation.featureTree,
            features: mutableSnapshot.document.features,
            sketches: mutableSnapshot.document.sketches,
            historyOrder: deleteDocumentHistoryOrderEntry(
              historyOrder,
              deletedItem,
            ),
          });
        ...
        const diagnostics: DeleteDocumentTargetResponse["diagnostics"] = [
          {
            ...
            severity: "info",
            message:
              "Mock kernel committed the generic deletion into the current revision.",
            target,
            detail: null,
          },
        ];
    ...
    if (request.target.kind === "body") {
      const target = request.target;
      if (
        !snapshot.document.bodies.some((body) => body.bodyId === target.bodyId)
      ) {
      ...
      return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
        ...
        mutableSnapshot.presentation.objects =
          mutableSnapshot.presentation.objects.filter(
            (item) =>
              item.target.kind !== "body" ||
              item.target.bodyId !== target.bodyId,
          );
        ...
        const diagnostics: DeleteDocumentTargetResponse["diagnostics"] = [
          {
            ...
            severity: "info",
            message:
              "Mock kernel committed the generic body deletion into the current revision.",
            target,
            detail: null,
          },
        ];
  ...
  async renameBody(request: RenameBodyRequest): Promise<RenameBodyResponse> {
    ...
    if (
      !snapshot.document.bodies.some((body) => body.bodyId === request.bodyId)
    ) {
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      for (const item of mutableSnapshot.presentation.objects) {
        if (
          item.target.kind === "body" &&
          item.target.bodyId === request.bodyId
        ) {
      ...
      for (const entityRecord of mutableSnapshot.presentation.entities) {
        if (
          entityRecord.target.kind === "body" &&
          entityRecord.target.bodyId === request.bodyId
        ) {
      ...
      const diagnostics: RenameBodyResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the body rename into the current revision.",
          target: bodyTarget,
          detail: null,
        },
      ];
  ...
  async reorderFeature(
    ...
    if (
      ...
      !snapshot.document.features.some(
        (feature) => feature.featureId === request.beforeFeatureId,
      )
    ) {
      const diagnostics = [
        createMissingFeatureAnchorDiagnostic(request.beforeFeatureId),
      ];
      return {
        contractVersion: CONTRACT_VERSION,
        documentId: request.documentId,
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      const fromIndex = mutableSnapshot.document.features.findIndex(
        (feature) => feature.featureId === request.featureId,
      );
      ...
      const targetIndex =
        request.beforeFeatureId === null
          ...
          : mutableSnapshot.document.features.findIndex(
              (feature) => feature.featureId === request.beforeFeatureId,
            );
      ...
      const diagnostics: ReorderFeatureResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the feature reorder into the current revision.",
          target: { kind: "feature", featureId: request.featureId },
          detail: null,
        },
      ];
  ...
  async reorderDocumentHistory(
    ...
    const currentOrder = createDocumentHistoryOrder(
      snapshot.presentation.documentHistory,
    );
    const itemKey = getDocumentHistoryOrderEntryKey(request.item);
    const beforeKey =
      request.beforeItem === null
        ? null
        : getDocumentHistoryOrderEntryKey(request.beforeItem);
    if (
      !currentOrder.some(
        (entry) => getDocumentHistoryOrderEntryKey(entry) === itemKey,
      )
    ...
    const historyOrder = reorderDocumentHistoryOrder(
      currentOrder,
      request.item,
      request.beforeItem,
    );
    ...
    if (historyOrder === null) {
      throw new Error(
        "Document history order changed during mock reorder preparation.",
      );
    }

    const orderedFeatures = reorderFeaturesByDocumentHistory(
      snapshot.document.features,
      historyOrder,
    );
    const dependencyDiagnostics = findDocumentHistoryOrderDependencyViolations(
      orderedFeatures,
      historyOrder,
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      mutableSnapshot.document.features = reorderFeaturesByDocumentHistory(
        mutableSnapshot.document.features,
        historyOrder,
      );
      mutableSnapshot.presentation.documentHistory = createDocumentHistoryItems(
      ...
      const diagnostics: ReorderDocumentHistoryResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the document history reorder into the current revision.",
          target:
            request.item.kind === "sketch"
          ...
          : [
              {
                ...
                target:
                  request.cursor.kind === "sketch"
                    ? {
                        kind: "sketch" as const,
                        sketchId: request.cursor.sketchId,
                      }
                    : null,
                detail: null,
              },
  ...
  async addDocumentVariable(
    ...
    const snapshot = await this.getSnapshot();
    const variableId =
      request.variableId ?? allocateDocumentVariableId(snapshot);
    ...
    if (!variableValidation.ok) {
      const diagnostics = createDocumentVariableExpressionDiagnostics(
        variableValidation.diagnostics,
      );
      ...
      const diagnostics: AddDocumentVariableResponse["diagnostics"] = [
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the document variable create request.",
          target: null,
          detail: null,
        },
      ];
  ...
  async updateDocumentVariable(
    ...
    if (
      !snapshot.document.variables.some(
    ...
    return this.mutateSnapshot((mutableSnapshot, nextRevisionId) => {
      ...
      const diagnostics: UpdateDocumentVariableResponse["diagnostics"] = [
        ...sketchRebuild.diagnostics,
        {
          ...
          severity: "info",
          message:
            "Mock kernel committed the document variable update request.",
          target: null,
          detail: null,
        },
      ];
  ...
  async evaluatePreview(
    ...
    const validation = resolvedDefinition.ok
      ? validateFeatureDefinitionAgainstSnapshot(
          resolvedDefinition.definition,
          snapshot,
        )
      : {
    ...
    return {
      ...
      documentId: request.documentId,
      revisionId:
        request.baseRevisionId === this.currentRevisionId
          ? request.baseRevisionId
          : this.currentRevisionId,
      ...
      render: {
        schemaVersion: RENDER_EXPORT_SCHEMA_VERSION,
        records:
          validation.accepted && resolvedDefinition.ok
            ? buildPreviewRenderables(resolvedDefinition.definition, snapshot)
            : [],
      },
      diagnostics: validation.diagnostics,
    };
  ...
  async resolveReference(
    ...
    assertSupportedModelingRequest(request);
    const snapshot = await this.getSnapshot();
    const entityRecord = snapshot.presentation.entities.find(
      (entry) =>
        getPrimitiveRefKey(entry.target) === getPrimitiveRefKey(request.target),
    );
        ...
        : [
            {
              ...
              severity: "error",
              message:
                "Requested durable reference does not resolve in the current snapshot.",
              target: request.target,
              detail: {
