...
function assertSupportedModelingRequest(
  ...
  if (request.documentId !== documentId) {
    throw new Error(
      `Unsupported document ${request.documentId}; expected ${documentId}.`,
    );
  }
}
...
function createMissingFeatureDiagnostic(
  featureId: FeatureId,
): ModelingDiagnostic {
...
function createUnsupportedDeleteTargetDiagnostic(
  target: DurableRef,
): ModelingDiagnostic {
  return createDiagnostic(
...
function createMissingReorderAnchorDiagnostic(
  featureId: FeatureId,
): ModelingDiagnostic {
  return createDiagnostic(
  ...
  return createDiagnostic(
    ...
    diagnostic.target?.kind === "entity"
      ? {
          kind: "sketchEntity",
          sketchId,
          entityId: diagnostic.target.entityId as SketchEntityId,
        }
      : diagnostic.target?.kind === "point"
        ? {
            kind: "sketchPoint",
            sketchId,
            pointId: diagnostic.target.pointId as SketchPointId,
          }
        : diagnostic.target?.kind === "region"
          ? {
              kind: "region",
              sketchId,
              regionId: diagnostic.target.regionId as RegionId,
            }
          : null,
  );
}
...
function createAuthoredHistoryRestoreOrder(
  ...
  const sketchById = new Set(
    document.sketches.map((sketch) => sketch.sketchId),
  );
  const featureById = new Set(
...
async function resolveGeometryAssetBlobs(
  ...
  const assetBlobs = new Map<
    AuthoredModelDocument["assets"]["records"][number]["hash"],
    Uint8Array
  >();
  if (!assetResolver) {
    return assetBlobs;
...
function featureOrdinalForLabel(
  ...
  return (
    state.features.filter((feature) => feature.definition.kind === kind)
      .length + 1
  );
}

function isValidFeatureCursor(
  state: OccAuthoringState,
  ...
  return state.features.some(
    (feature) => feature.featureId === cursor.featureId,
  );
}
    ...
    historyOrder
      ...
      .filter(
        (
          ...
          OccAuthoringState["historyOrder"][number],
          { kind: "feature" }
        > => item.kind === "feature",
      )
...
function getAppliedFeatures(
  ...
  if (!historyOrder) {
    ...
    const cursorIndex = features.findIndex(
      (feature) => feature.featureId === cursor.featureId,
    );
...
function uniqueTargets(
  targets: readonly NonNullable<ModelingDiagnostic["target"]>[],
) {
  const seen = new Set<string>();
...
function bodyIdLooksOwnedByFeature(bodyId: BodyId, featureId: FeatureId) {
  return (
    bodyId === `body_${featureId}` || bodyId.startsWith(`body_${featureId}_`)
  );
}
...
function targetBlockedByFailedFeature(
  target: DurableRef,
  failedFeature: FailedFeatureRecord,
) {
  return (
    targetLooksOwnedByFeature(target, failedFeature.featureId) ||
    failedFeature.affectedTargets.some((affectedTarget) =>
      targetsOverlap(affectedTarget, target),
    )
  );
}
...
function createFailedFeatureRecord(
  feature: OccAuthoringFeatureRecord,
): FailedFeatureRecord {
  return {
    featureId: feature.featureId,
    ...
    affectedTargets: uniqueTargets(
      getFeatureConsumedTargets(feature.definition),
    ),
  };
}

function getPersistentAuthoringDiagnostics(
...
function mergeModelingDiagnostics(
  ...
  const seen = new Set(
    existing.map(
      (diagnostic) =>
        `${diagnostic.code}|${diagnostic.featureId ?? ""}|${diagnostic.message}`,
    ),
  );
...
function getNewInvalidatedTargets(
  ...
  const previousKeys = new Set(
    previous.referenceState.invalidatedReferencesByKey.keys(),
  );
...
function buildPreviewRenderRecords(
  records: ReturnType<
    typeof buildOccWorkspaceSnapshot
...
function filterPreviewInvalidationDiagnostics(
  ...
  const previewInvalidationKeys = new Set(
    getNewInvalidatedTargets(previousState, previewState).map((target) =>
...
function createDefaultSolverCorrelation(
  sketchId: SketchId,
  revisionId: RevisionId,
) {
  ...
  return {
    ...
    projectionRequestId:
      `request_project_${sketchSuffix}_${revisionSuffix}` as const,
    validationRequestId:
      `request_validate_${sketchSuffix}_${revisionSuffix}` as const,
    solveRequestId: `request_solve_${sketchSuffix}_${revisionSuffix}` as const,
    regionRequestId:
      `request_regions_${sketchSuffix}_${revisionSuffix}` as const,
  };
}
...
export class OpenCascadeKernelAdapter implements ModelingKernelAdapter {
  ...
  private nativeTopologyBodyPayloadCache = new Map<
    string,
    CachedNativeTopologyBodyPayload
  >();
  ...
  private async restoreAuthoredModelDocumentOnMainThread(
    ...
    const features: OccAuthoringFeatureRecord[] = document.featureOrder
      ...
      .filter((feature): feature is AuthoredModelDocument["features"][number] =>
        Boolean(feature),
      )
      .map((feature) => ({
        featureId: feature.featureId,
    ...
    const featureRecordById = new Map(
      features.map((feature) => [feature.featureId, feature]),
    );
  ...
  async exportAuthoredModelDocument(
    ...
    if (this.workerRestoredDocument) {
      if (this.workerRestoredDocument.document.documentId !== documentId) {
        throw new Error(
          `OCC authored export requested document ${documentId}, but active document is ${this.workerRestoredDocument.document.documentId}.`,
        );
      }
    ...
    if (runtimeState.authoringState.documentId !== documentId) {
      throw new Error(
        `OCC authored export requested document ${documentId}, but active document is ${runtimeState.authoringState.documentId}.`,
      );
    }

    return createAuthoredModelDocumentFromAuthoringState(
  ...
  private buildNextAuthoringState(
    ...
    const rebuiltFeatures = new Map(
      current.features.map((feature) => [feature.featureId, feature]),
    );
  ...
  private requireLiveConstructionPlane(
    ...
    if (!plane) {
      throw new Error(
        `Construction plane ${constructionId} does not resolve in the current OCC authoring state.`,
      );
    }
  ...
  private validateSketchPlaneSupport(
    state: OccAuthoringState,
  ...
  private buildConflictResult(
    ...
    const diagnostics = [
      createRevisionConflictDiagnostic(baseRevisionId, currentRevisionId),
    ];
  ...
  private buildAcceptedResult(
    ...
    const invalidatedTargets = getNewInvalidatedTargets(
      previousState,
      nextState,
    );
    const diagnostics = buildOccSnapshotDiagnostics(
      nextState,
      extraDiagnostics,
    );
  ...
  private tryBuildNextAuthoringState(
    ...
    const diagnostics: ModelingDiagnostic[] = [];
    const dependencyOrderDiagnostics = new Map<
      FeatureId,
      ModelingDiagnostic[]
    >();

    for (const diagnostic of findDocumentHistoryOrderDependencyViolations(
      features,
      baseState.historyOrder,
      ...
      const featureDiagnostics =
        dependencyOrderDiagnostics.get(diagnostic.target.featureId) ?? [];
      featureDiagnostics.push(diagnostic);
      dependencyOrderDiagnostics.set(
        diagnostic.target.featureId,
        featureDiagnostics,
      );
    }
    ...
    for (const feature of getAppliedFeatures(
      ...
      const orderingDiagnostics = dependencyOrderDiagnostics.get(
        feature.featureId,
      );
      ...
      const invalidConsumedTargetDiagnostics =
        collectInvalidConsumedTargetDiagnostics(current, feature);
  ...
  private createNativeTopologyUnavailableResult<TPayload>(
    ...
    const capability = probeOpenCascadeNativeTopologyKernelCapabilities(
      state.oc,
    );
  ...
  private buildNativeTopologyPayloadForState(
    ...
    const nativeHost =
      this.getNativeTopologyHost<OccNativeTopologyPayload>(state);
    ...
    const builder = options.useCommittedShapeTransaction
      ? nativeHost.host.CadaraExecuteNativeFeatureTransaction
          ?.BuildCommittedShapePayload
      : nativeHost.host.CadaraBuildNativeTopologyPayload?.BuildJson;
    if (!builder) {
      return this.createNativeTopologyUnavailableResult<OccNativeTopologyPayload>(
        state,
      );
    }
  ...
  async getDocumentSnapshot(
    ...
    if (this.workerSnapshotClient) {
      return this.workerSnapshotClient.getDocumentSnapshot(
        request,
  ...
  async buildNativeTopologySnapshot(
    ...
    if (this.workerSnapshotClient) {
      return this.workerSnapshotClient.buildNativeTopologySnapshot(
        request,
        lodTierId,
      );
    }
  ...
  async buildNativeBooleanFeatureTransactionPayload(
    ...
    if (documentId !== this.documentId) {
      throw new Error(
        `OCC native boolean transaction requested document ${documentId}, but active document is ${this.documentId}.`,
      );
    }
    ...
    const createDiagnosticPayload = (
      diagnostics: readonly ReturnType<typeof createNativeTopologyDiagnostic>[],
    ) => {
      const payload = createOccNativeTopologyPayloadFromShimPayloads({
        revisionId: state.revisionId,
        lodTierId: null,
      ...
      return createDiagnosticPayload([
        createNativeTopologyDiagnostic(
          ...
          `Request revision ${baseRevisionId} does not match current revision ${state.revisionId}.`,
          {
            kind: "revisionConflict",
            expectedRevisionId: baseRevisionId,
            actualRevisionId: state.revisionId,
          },
    ...
    const leftBody = state.bodies.find(
      (candidate) => candidate.bodyId === leftBodyId,
    );
    const rightBody = state.bodies.find(
      (candidate) => candidate.bodyId === rightBodyId,
    );
    ...
    const nativePayload = parseNativeShimPayloadJson(
      transactionResult.PayloadJson(),
    );
    const transactionHistory = parseNativeFeatureTransactionHistoryJson(
      transactionResult.HistoryJson(),
    );
    const payload = createOccNativeTopologyPayloadFromShimPayloads({
      revisionId: state.revisionId,
      lodTierId: tier.id,
  ...
  async buildNativeMeshExportPayload(
    ...
    if (documentId !== this.documentId) {
      throw new Error(
        `OCC native mesh export requested document ${documentId}, but active document is ${this.documentId}.`,
      );
    }

    if (this.workerSnapshotClient) {
      return this.workerSnapshotClient.buildNativeMeshExportPayload(
        documentId,
        baseRevisionId,
        target,
        options,
    ...
    const nativeHost =
      this.getNativeTopologyHost<OccNativeMeshExportPayload>(state);
    if (!nativeHost.ok) {
      return nativeHost.result;
    }

    const builder =
      nativeHost.host.CadaraBuildNativeMeshExportPayload?.BuildJson;
    if (!builder) {
      return this.createNativeTopologyUnavailableResult<OccNativeMeshExportPayload>(
        state,
      );
    }
  ...
  async buildNativeExactBrepPayload(
    ...
    if (documentId !== this.documentId) {
      throw new Error(
        `OCC native exact B-rep requested document ${documentId}, but active document is ${this.documentId}.`,
      );
    }

    if (this.workerSnapshotClient) {
      return this.workerSnapshotClient.buildNativeExactBrepPayload(
        documentId,
        baseRevisionId,
        target,
      );
    ...
    const nativeHost =
      this.getNativeTopologyHost<OccNativeExactBrepPayload>(state);
    if (!nativeHost.ok) {
      return nativeHost.result;
    }

    const builder =
      nativeHost.host.CadaraBuildNativeExactBrepPayload?.BuildJson;
    if (!builder) {
      return this.createNativeTopologyUnavailableResult<OccNativeExactBrepPayload>(
        state,
      );
    }
  ...
  async commitSketch(
    ...
    const sketchId =
      request.sketchId ?? allocateSketchId(runtimeState.authoringState);

    if (request.baseRevisionId !== currentRevisionId) {
      const conflict = this.buildConflictResult(
    ...
    const requestDiagnostics = this.validateCommitSketchRequest(
      runtimeState.authoringState,
      request,
    );
    ...
    const nextSketches = !isNewSketch
      ? runtimeState.authoringState.sketches.map((entry) =>
          entry.sketchId === sketchId ? snapshotRecord : entry,
        )
  ...
  async createFeature(
    ...
    const feature: OccAuthoringFeatureRecord = {
      ...
      label:
        ...
        `${capitalizeFeatureKind(request.definition.kind)} ${featureOrdinalForLabel(runtimeState.authoringState, request.definition.kind)}`,
      suppressed: false,
  ...
  async deleteFeature(
    ...
    const nextHistoryOrder = deleteDocumentHistoryOrderEntry(
      runtimeState.authoringState.historyOrder,
      deletedItem,
    );
    const nextCursor = repairCursorAfterHistoryDeletion(
      runtimeState.authoringState.cursor,
      nextHistoryOrder,
    );
  ...
  async deleteTarget(
    ...
    if (request.target.kind === "feature" || request.target.kind === "sketch") {
      ...
      if (
        !runtimeState.authoringState.historyOrder.some(
          (entry) => getDocumentHistoryOrderEntryKey(entry) === itemKey,
        )
      ) {
        const diagnostic =
      ...
      const nextFeatures =
        target.kind === "feature"
          ? runtimeState.authoringState.features.filter(
              (feature) => feature.featureId !== target.featureId,
            )
          : runtimeState.authoringState.features;
      const nextSketches =
        target.kind === "sketch"
          ? runtimeState.authoringState.sketches.filter(
      ...
      const deletedFeature =
        target.kind === "feature"
          ? runtimeState.authoringState.features.find(
              (feature) => feature.featureId === target.featureId,
            )
          : null;
      const accepted = this.buildAcceptedResult(
  ...
  async renameBody(request: RenameBodyRequest): Promise<RenameBodyResponse> {
    ...
    if (
      !runtimeState.authoringState.bodies.some(
        (body) => body.bodyId === request.bodyId,
      )
    ) {
      const rejected = this.buildRejectedResult(
        request.baseRevisionId,
  ...
  async reorderFeature(
    ...
    if (
      ...
      !runtimeState.authoringState.features.some(
    ...
    if (!movedFeature) {
      throw new Error(
        `Feature ${request.featureId} disappeared during reorder preparation.`,
      );
    }
    ...
    const insertIndex =
      request.beforeFeatureId === null
        ? nextFeatures.length
        : nextFeatures.findIndex(
            (feature) => feature.featureId === request.beforeFeatureId,
          );

    nextFeatures.splice(
      insertIndex < 0 ? nextFeatures.length : insertIndex,
      0,
      movedFeature,
    );
  ...
  async reorderDocumentHistory(
    ...
    const nextHistoryOrder = reorderDocumentHistoryOrder(
      currentOrder,
      request.item,
      request.beforeItem,
    );
    ...
    const nextFeatures = reorderFeaturesByDocumentHistory(
      runtimeState.authoringState.features,
      nextHistoryOrder,
    );
  ...
  async addDocumentVariable(
    ...
    const variableId =
      request.variableId ??
      allocateDocumentVariableId(runtimeState.authoringState);
  ...
  async updateDocumentVariable(
    ...
    if (
      !runtimeState.authoringState.variables.some(
    ...
    const candidateVariables = runtimeState.authoringState.variables.map(
      (variable) =>
  ...
  async evaluatePreview(
    ...
    const runtimeState = await this.getRuntimeState();
    const currentRevisionId = this.getCurrentRevisionId(runtimeState);
    const previewFeatureId =
      `feature_preview_${request.previewId}` as FeatureId;
    ...
    try {
      ...
      return {
        ...
        render: {
          ...
          records: buildPreviewRenderRecords(
  ...
  async getExportCapabilities(
    ...
    if (this.workerSnapshotClient) {
      return this.workerSnapshotClient.getExportCapabilities(
        this.documentId,
        baseRevisionId,
      );
    }
