  ...
  switch (value.kind) {
    ...
    case "face":
      if (isString(value.bodyId) && isString(value.faceId)) {
        return {
          kind: "face",
          bodyId: value.bodyId as BodyId,
          faceId: value.faceId as FaceId,
        };
      }
      break;
    case "edge":
    ...
    case "vertex":
      if (isString(value.bodyId) && isString(value.vertexId)) {
        return {
          kind: "vertex",
          bodyId: value.bodyId as BodyId,
          vertexId: value.vertexId as VertexId,
        };
      }
      break;
    case "loop":
      if (isString(value.bodyId) && isString(value.loopId)) {
        return {
          kind: "loop",
          bodyId: value.bodyId as import("@/contracts/shared/ids").BodyId,
          loopId: value.loopId as import("@/contracts/shared/ids").LoopId,
        };
      }
      break;
    case "sketch":
    ...
    case "construction":
      if (isString(value.constructionId)) {
        return {
          kind: "construction",
          constructionId: value.constructionId as ConstructionId,
        };
      }
      break;
    case "region":
...
export function assertSketchPlaneSupportRef(
  value: unknown,
...
export function assertExtrudeProfileRefs(
  ...
  if (value.length === 0) {
    throw new Error(
      `${featureLabel} profiles must include at least one explicit region or planar face reference.`,
    );
  }
  ...
  for (const profile of profiles) {
    ...
    if (seen.has(key)) {
      throw new Error(
        `${featureLabel} profiles must not contain duplicate profile references.`,
      );
    }
    seen.add(key);
  }
...
export function assertShellFaceRef(
  value: unknown,
): Extract<PrimitiveRef, { kind: "face" }> {
  const target = assertPrimitiveRef(value);
...
export function normalizeUpToOffset(
  value: unknown,
