...
import {
  ...
  type OccFeaturePresentationArtifacts,
...
function buildConstructionOutlinePoints(
...
export function executePlaneFeature(
  ...
  if (parameters.reference.target.kind === "construction") {
    ...
    if (!sourceConstruction) {
      throw new Error(
        `Construction plane ${sourceConstructionId} does not resolve in the current OCC authoring state.`,
      );
    }
  }
  ...
  const plane: SketchPlaneDefinition =
    parameters.reference.target.kind === "construction"
      ? {
          support: { kind: "construction", constructionId },
          frame: requireConstructionPlaneDefinition(
            context,
            parameters.reference.target.constructionId,
          ).frame,
          key: null,
        }
