...
function getThickenThickness(
  ...
  if (
    ...
    throw new Error(
      "advanced-feature-unsupported-kernel-case: OCC thicken requires a positive thickness option.",
    );
  }
...
function getThickenSide(
  ...
  if (side === "symmetric") {
    throw new Error(
      "advanced-feature-unsupported-kernel-case: OCC thicken does not support symmetric side mode yet.",
    );
  }

  throw new Error(
    "advanced-feature-unsupported-kernel-case: OCC thicken side must be oneSide.",
  );
}

function getThickenDirection(
  definition: AdvancedSolidFeatureDefinition & { kind: "thicken" },
) {
  ...
  throw new Error(
    "advanced-feature-unsupported-kernel-case: OCC thicken direction must be positive or negative.",
  );
}

function getThickenFaceTargets(
  definition: AdvancedSolidFeatureDefinition & { kind: "thicken" },
) {
  ...
  if (faceTargets.length !== 1) {
    throw new Error(
      "advanced-feature-unsupported-kernel-case: OCC thicken requires exactly one face target in the initial implementation.",
    );
  }
  ...
  for (const target of faceTargets) {
    if (target.kind !== "face") {
      throw new Error(
        "advanced-feature-unsupported-kernel-case: OCC thicken face participants must be durable face targets.",
      );
    }
  }
...
function buildThickenFeatureShape(
  ...
  try {
    ...
    throw new Error(
      "advanced-feature-unsupported-kernel-case: OCC thicken requires a planar face target.",
    );
  }
  ...
  if (!prism.IsDone()) {
    throw new Error(
      "advanced-feature-unsupported-kernel-case: OCC thicken prism build failed.",
    );
  }
...
function getThickenBooleanPolicy(
  definition: AdvancedSolidFeatureDefinition & { kind: "thicken" },
): {
  ...
  throw new Error(
    "advanced-feature-unsupported-kernel-case: OCC thicken does not support boolean composition yet.",
  );
}
