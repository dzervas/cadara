...
export type OpenCascadeEntrySpecifier =
  | "opencascade.js"
...
export function createOpenCascadeInitializerFromMainJS(
  ...
  return async ({
    ...
    const dynamicLibraryHost = oc as OpenCascadeInstance &
      Partial<OpenCascadeDynamicLibraryHost>;

    for (const lib of libs) {
      if (!dynamicLibraryHost.loadDynamicLibrary) {
        throw new Error(
          "OpenCascade runtime does not support dynamic library loading.",
        );
      }
...
function assertRequiredOpenCascadeBindings(oc: OpenCascadeInstance) {
  ...
  try {
    ...
    if (!faceBuilder.IsDone()) {
      throw new Error(
        "Custom OpenCascade build cannot construct a planar face from a closed wire.",
      );
    }
  } finally {
    faceBuilder?.delete?.();
...
export function probeOpenCascadeNativeTopologyKernelCapabilities(
  ...
  return probeNativeTopologyKernelCapabilities(
    oc as unknown as Parameters<
      typeof probeNativeTopologyKernelCapabilities
    >[0],
  );
}
...
export function assertOpenCascadeNativeTopologyKernelCapabilities(
  ...
  const missingEntrypoints = getMissingNativeTopologyKernelEntrypoints(
    oc as unknown as Parameters<
      typeof getMissingNativeTopologyKernelEntrypoints
    >[0],
  );
...
async function loadBrowserOpenCascadeModule(): Promise<OpenCascadeFactoryModule> {
  ...
  if (typeof customMainJS !== "function") {
    throw new Error(
      "Custom OpenCascade build did not expose a default initializer module.",
    );
  }
...
async function loadNodeOpenCascadeModule(): Promise<OpenCascadeFactoryModule> {
  const allowLegacyTopologyEnumerationForNodeTests = async (
    initializer: OpenCascadeInitializer,
  ) => {
  ...
  try {
    ...
    const module = (await import(
      /* @vite-ignore */ nodeEntrySpecifier
    )) as OpenCascadeFactoryModule;
    const initializer = resolveOpenCascadeInitializer(module);

    return {
      default: (settings = {}) =>
        allowLegacyTopologyEnumerationForNodeTests(() => initializer(settings)),
    };
  } catch {
    const legacyMainSpecifier = "opencascade.js/dist/" + "opencascade.wasm.js";
    const legacyWasmSpecifier =
        ...
        const initializer = async () =>
          defaultInitializer({
            ...
            module: {
              ...settings.module,
              wasmBinary:
                settings.module?.wasmBinary ??
                (await readNodeOpenCascadeWasmBinary(wasmPath)),
            },
          });
...
function resolveOpenCascadeInitializer(
  module: OpenCascadeFactoryModule,
): OpenCascadeInitializer {
  const initializer = module.default ?? module.initOpenCascade;

  if (typeof initializer !== "function") {
    throw new Error(
      "OpenCascade module did not expose an initializer function.",
    );
  }
...
export async function loadDefaultOpenCascadeFactory(
  ...
  const loadModule =
    entrySpecifier === "opencascade.js/dist/node.js"
