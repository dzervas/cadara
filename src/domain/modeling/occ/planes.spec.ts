...
test("src/domain/modeling/occ/planes.spec.ts", async () => {
  ...
  async function testWorldAndSketchTransformsRoundTrip() {
    ...
    assertClose(
      ...
      sketchPoint[0],
      1e-9,
      "Sketch X should round-trip through world space.",
    );
    assertClose(
      recovered[1],
      sketchPoint[1],
      1e-9,
      "Sketch Y should round-trip through world space.",
    );
  ...
  async function testOccAxisAndPlaneBuildersPreserveFrameData() {
    ...
    assertVecClose(
      [
        normalAxis.Location().X(),
        normalAxis.Location().Y(),
        normalAxis.Location().Z(),
      ],
      plane.frame.origin,
      1e-9,
    ...
    assertVecClose(
      [
        normalAxis.Direction().X(),
        normalAxis.Direction().Y(),
        normalAxis.Direction().Z(),
      ],
      plane.frame.normal,
      1e-9,
    ...
    assertVecClose(
      [
        gpPlane.Position().Location().X(),
        gpPlane.Position().Location().Y(),
        gpPlane.Position().Location().Z(),
      ],
      plane.frame.origin,
      1e-9,
    ...
    assertVecClose(
      [
        gpPlane.Position().Direction().X(),
        gpPlane.Position().Direction().Y(),
        gpPlane.Position().Direction().Z(),
      ],
      plane.frame.normal,
      1e-9,
  ...
  async function testPlanarFaceExtractionPreservesExplicitFrameData() {
    ...
    assertVecClose(
      extracted.frame.origin,
      plane.frame.origin,
      1e-9,
      "Planar-face extraction should preserve plane origin",
    );
    assertVecClose(
      extracted.frame.normal,
      plane.frame.normal,
      1e-9,
      "Planar-face extraction should preserve plane normal",
    );
    assertVecClose(
      extracted.frame.xAxis,
      plane.frame.xAxis,
      1e-9,
      "Planar-face extraction should preserve plane xAxis",
    );
    assertVecClose(
      extracted.frame.yAxis,
      plane.frame.yAxis,
      1e-9,
      "Planar-face extraction should preserve plane yAxis",
    );
  }
  ...
  async function testFaceBackedConstructionPlaneHelpersReusePlanarExtraction() {
    ...
    const constructionPlane = buildConstructionPlaneFromPlanarFace(
      oc,
      face,
      support.faceId,
      support,
    );
    const extrusionNormal = getExtrusionNormalForPlanarFace(
      oc,
      face,
      "positive",
    );
    ...
    assertVecClose(
      constructionPlane.frame.origin,
      plane.frame.origin,
      1e-9,
      "Face-backed construction planes should preserve extracted origin",
    );
    assertVecClose(
      constructionPlane.frame.normal,
      plane.frame.normal,
      1e-9,
      "Face-backed construction planes should preserve extracted normal",
    );
    assertVecClose(
      constructionPlane.frame.xAxis,
      plane.frame.xAxis,
      1e-9,
      "Face-backed construction planes should preserve extracted xAxis",
    );
    assertVecClose(
      constructionPlane.frame.yAxis,
      plane.frame.yAxis,
      1e-9,
      "Face-backed construction planes should preserve extracted yAxis",
    );
    assertVecClose(
      extrusionNormal,
      plane.frame.normal,
      1e-9,
      "Face-backed extrusion normals should preserve extracted planar normals",
    );
  }
  ...
  async function testPlanarFaceExtractionRejectsNonPlanarFaces() {
    const oc = await getDefaultOpenCascadeInstance();
    const plane = createRotatedSketchPlane();
    const cylinder = new oc.gp_Cylinder_2(toGpAx3(oc, plane), 2);
    ...
    try {
      extractPlanarFaceData(
        oc,
        pipeFaceBuilder.Face(),
        "Expected non-planar rejection.",
      );
    } catch (error) {
