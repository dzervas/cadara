...
test("document snapshot view resolves selection details, contributing features, and selection catalogs", async () => {
  ...
  const featureTarget = {
    kind: "feature",
    featureId: snapshot.document.features[0]!.featureId,
  } as const;
  const bodyTarget = {
    kind: "body",
    bodyId: snapshot.document.bodies[0]!.bodyId,
  } as const;
  const faceTarget = {
    kind: "face",
    bodyId: snapshot.document.bodies[0]!.bodyId,
  ...
  const missingTarget = {
    kind: "feature",
    featureId: "feature_missing" as typeof featureTarget.featureId,
  } as const;
  ...
  expectTrue(
    catalog.selectableTargetKeys.includes(
      ...
      catalog.constructionPlaneKeys.includes(
        "construction:construction_plane-xy",
      ) &&
  ...
  const brokenSnapshot = {
    ...
    presentation: {
      ...
      entities: snapshot.presentation.entities.map((entity) =>
        entity.target.kind === "feature" &&
        entity.target.featureId === featureTarget.featureId
          ? {
              ...entity,
              relatedTargets: [
                {
                  kind: "feature",
                  featureId:
                    "feature_missing" as typeof featureTarget.featureId,
                },
              ],
            }
          : entity,
