      ...
      <div className="pointer-events-none absolute inset-0 z-10">
        ...
        >
          {overlays.map((overlay) =>
            renderOverlayGeometry(overlay, projectionById, dragCallbacks),
          )}
        </svg>
        {overlays.map((overlay) => {
          const projection = projectionById.get(
            getOverlayProjectionId(overlay.id),
          );
...
function getOverlayClassName(overlay: SketchToolOverlayDescriptor) {
  const baseClassName =
    "pointer-events-none absolute max-w-[180px] whitespace-nowrap rounded border px-2 py-1 text-[11px] leading-none shadow-[var(--cad-panel-shadow)]";
  ...
  switch (overlay.kind) {
        ...
        <g key={overlay.id} data-sketch-viewport-geometry="dimensionLine">
          {(overlay.extensionLines ?? []).map((line) => {
            const start = projectionById.get(
              getOverlayGeometryProjectionId(line.id, "start"),
            );
            const end = projectionById.get(
              getOverlayGeometryProjectionId(line.id, "end"),
            );
          ...
          {renderProjectedLine({
            id: overlay.id,
            start: projectionById.get(
              getOverlayGeometryProjectionId(overlay.id, "start"),
            ),
            end: projectionById.get(
              getOverlayGeometryProjectionId(overlay.id, "end"),
            ),
    ...
    case "snapIndicator":
      return renderSnapIndicator(
        overlay,
        projectionById.get(getOverlayProjectionId(overlay.id)),
      );
    default:
      return null;
  }
...
function renderSnapIndicator(
  ...
  return (
    <g
      key={overlay.id}
      data-sketch-viewport-geometry="snapIndicator"
      data-sketch-snap-kind={overlay.candidateKind}
    >
      {overlay.glyphKind === "midpoint" ? (
        ...
        />
      ) : overlay.glyphKind === "horizontal" ||
        overlay.glyphKind === "vertical" ? (
        <line
...
function renderProjectedArc(
  ...
  const center = projectionById.get(
    getOverlayGeometryProjectionId(id, "center"),
  );
  ...
  const witnessLines = (overlay.witnessLines ?? []).map((line) => {
    const witnessStart = projectionById.get(
      getOverlayGeometryProjectionId(line.id, "start"),
    );
    const witnessEnd = projectionById.get(
      getOverlayGeometryProjectionId(line.id, "end"),
    );
...
function getArcSweepDelta(
  ...
  return minorDelta < 0 ? minorDelta + Math.PI * 2 : minorDelta - Math.PI * 2;
}
...
function createCenteredArcPath(input: {
  ...
  const segmentCount = Math.max(
    8,
    Math.ceil(Math.abs(input.delta) / (Math.PI / 24)),
  );
