...
export function ImportInspector({ onCommit }: ImportInspectorProps) {
  ...
  const provider = activeImportSession
    ? importProviders.getById(activeImportSession.providerId)
    : null;
  const formSchema = useMemo(
    () =>
      activeImportSession
        ? withSessionDiagnostics(
            activeImportSession.formSchema,
            activeImportSession.diagnostics,
          )
        : null,
    [activeImportSession],
  );
  ...
  const commitDisabled =
    hasBlockingErrors(formSchema) || activeCommand?.phase === "awaitingEffect";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text mt={8} size="sm" fw={500} c="var(--workbench-shell-text)">
          {provider?.label ?? "Import"}
        </Text>
      </header>
          ...
          <section key={section.id} className="space-y-2">
            ...
            {section.fields.map((field) => (
              <FeatureFormFieldRenderer
                ...
                onPatch={(patch) =>
                  dispatch({ type: "import.selectionPatched", patch })
                }
              />
            ))}
          </section>
      ...
      <footer
        ...
        style={{ borderTop: "1px solid var(--workbench-shell-border)" }}
      >
        <Button
          type="button"
...
function hasBlockingErrors(schema: FeatureEditorFormSchema) {
  return schema.sections.some((section) =>
    section.fields.some(fieldHasBlockingError),
  );
}
...
function fieldHasBlockingError(field: FeatureEditorFormField): boolean {
  ...
  if (field.kind === "discriminatedOptionGroup") {
    return (
      fieldHasBlockingError(field.discriminant) ||
      field.variants.some((variant) =>
        variant.fields.some(fieldHasBlockingError),
      )
    );
  }
