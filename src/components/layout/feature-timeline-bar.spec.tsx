...
test("src/components/layout/feature-timeline-bar.spec.tsx", async () => {
  ...
  expectTrue(
    !variableSidebarMarkup.includes(
      'aria-label="Variable name variable_width"',
    ),
  ...
  expectTrue(
    featureMenuDescriptors
      ...
      .join("|") ===
      "Edit|Rename|Suppress|Roll History Here|Roll To End|Delete",
  ...
  expectTrue(
    suppressedFeatureMenuDescriptors
      ...
      .map((entry) => entry.label)
      .join("|") ===
      "Edit|Rename|Unsuppress|Roll History Here|Roll To End|Delete",
  ...
  expectTrue(
    tailMenuDescriptors.find((entry) => entry.id === "roll-to-end")
      ?.disabled === true,
  ...
  expectTrue(
    erroredTimelineMarkup.match(/data-delete-supported="true"/g)?.length ===
      erroredTimelineSnapshot.presentation.documentHistory.length,
