...
export function buildImportFilePickerConfiguration(
  ...
  const accept = normalized.reduce<Record<string, string[]>>(
    (result, entry) => {
...
export async function showOpenImportFilePicker(input: {
  ...
  const environment: ImportFilePickerEnvironment =
    input.environment ?? (globalThis as unknown as ImportFilePickerEnvironment);
