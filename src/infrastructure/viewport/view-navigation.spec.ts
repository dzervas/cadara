...
test("src/infrastructure/viewport/view-navigation.spec.ts", async () => {
...

  {
    ...
    approxVector(
      getViewNavigationDirection("frontRightTop"),
...

  {
    ...
    approxVector(
      camera.position,
      new THREE.Vector3(2, -3, 1).add(
        new THREE.Vector3(0, -1, 0).multiplyScalar(expectedDistance),
      ),
    );
    expect(updateCalls).toBe(1);
  }

  {
    ...
    const expectedDirection = new THREE.Vector3(1, -1, 1)
      .normalize()
      .multiplyScalar(expectedDistance);
    ...
    approxVector(
      camera.position,
      controls.target.clone().add(expectedDirection),
    );
    expect(updateCalls).toBe(1);
  }
});
