...
test("src/infrastructure/viewport/view-cube-navigation.spec.ts", async () => {
  ...
  expect(VIEW_CUBE_FACE_TARGETS.map(({ label }) => label)).toEqual([
    "Front",
    "Back",
    "Right",
    "Left",
  ...
  expect(
    VIEW_CUBE_FACE_TARGETS.filter(
      ({ presetId }) => presetId !== "top" && presetId !== "bottom",
    ).every(
  ...
  expect(
    VIEW_CUBE_FACE_TARGETS.find(({ presetId }) => presetId === "bottom")
      ?.labelUp,
  ).toEqual([0, -1, 0]);
  expect(
