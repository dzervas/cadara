...
} from "@/domain/modeling/memory-document-repository";
...
test("src/infrastructure/workers/document-sync-worker-client.spec.ts", async () => {
  ...
  async function testWorkerRuntimeShell() {
    ...
    await handle({
      kind: "normalize",
      requestId:
        "request_document_sync_normalize" as DocumentSyncWorkerRequest["requestId"],
      document: seed,
      metadata: {
        documentId: seed.documentId,
        heads: ["head_normalized"],
        source: "restore",
      },
    });
    expect(
    ...
    expect(
      posted.some(
        (message) =>
          ...
          message.requestId === "request_document_sync_asset_mutate" &&
          message.result.ok,
      ),
  ...
  async function testWorkerRuntimeStorageResetAndBindingFailures() {
    ...
    await handle({
      kind: "load",
      requestId:
        "request_document_sync_storage_load" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
    ...
    await handle({
      ...
      requestId:
        "request_document_sync_asset_bytes_missing" as DocumentSyncWorkerRequest["requestId"],
      hash: "sha256:missing" as DocumentSyncWorkerRequest["kind"] extends never
        ? never
        : never,
    });
    expect(
    ...
    await handle({
      kind: "bindFileHandle",
      requestId:
        "request_document_sync_bind_persist_unavailable" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
      ...
      posted.some((message) => message.kind === "fileHandleBound") &&
        posted.some(
          (message) =>
            ...
            message.status.kind === "persistent-binding-unavailable",
        ) &&
        posted.some(
          (message) =>
            message.kind === "writeStatusChanged" &&
    ...
    await handle({
      kind: "bindFileHandle",
      requestId:
        "request_document_sync_bind_failure" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
      handle: writableHandle.handle,
      metadata: {
        documentId: seed.documentId,
  ...
  async function testWorkerRuntimeFileBindingAutosyncAndPermissionFailures() {
    ...
    await handle({
      kind: "load",
      requestId:
        "request_document_sync_file_load" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
      seedDocument: seed,
    });
    ...
    await handle({
      kind: "bindFileHandle",
      requestId:
        "request_document_sync_file_bind" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
      handle: writableHandle.handle,
      metadata: {
        documentId: seed.documentId,
    ...
    await handle({
      kind: "bindFileHandle",
      requestId:
        "request_document_sync_file_bind_denied" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
    ...
    await handle({
      kind: "bindFileHandle",
      requestId:
        "request_document_sync_file_bind_failed" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
  ...
  async function testWorkerRuntimeLoadReReadsBoundFilesystemFile() {
    ...
    const authoritativeFileDocument = withBodyLabel(
      seed,
    ...
    await handle({
      kind: "load",
      requestId:
        "request_document_sync_file_backed_load" as DocumentSyncWorkerRequest["requestId"],
      documentId: seed.documentId,
      seedDocument: seed,
    });

    const loaded = posted.find(
      (
        message,
      ): message is Extract<DocumentSyncWorkerResponse, { kind: "loaded" }> =>
        message.kind === "loaded" &&
  ...
  async function testMatchingLinkedFileReusesCachedLoadResult() {
    ...
    await handle(
      createLoadRequest(seed, "request_document_sync_matching_linked_load"),
    );
  ...
  async function testMatchingLinkedFileIgnoresSerializationDifferences() {
    ...
    await handle(
      createLoadRequest(seed, "request_document_sync_serialized_linked_load"),
    );
  ...
  async function testMatchingLinkedFileNormalizesDocumentIdBeforeComparison() {
    ...
    const diskDocument = {
      ...seed,
      documentId:
        "doc_different_disk_identity" as AuthoredModelDocument["documentId"],
    };
    const repository = new TrackingMemoryDocumentRepository([seed]);
    const bindingStore = new MemoryLocalFileBindingStore();
    const posted: DocumentSyncWorkerResponse[] = [];
  ...
  async function testChangedLinkedFileRefreshesRepositoryWithoutAutosyncWrite() {
    ...
    await handle(
      createLoadRequest(seed, "request_document_sync_changed_linked_load"),
    );
  ...
  async function testInvalidAndUnreadableLinkedFilesDoNotReuseCache() {
    ...
    const invalidRepository = new TrackingMemoryDocumentRepository([
    ...
    const invalidLoaded = findLoaded(
      invalidPosted,
  ...
  async function testBrowserOnlyLoadBypassesLinkedFileComparison() {
    const seed = await createSeedAuthoredModelDocument();
    const cachedBrowserDocument = withBodyLabel(
      seed,
      "Browser Only Cached State",
    );
    const repository = new TrackingMemoryDocumentRepository([
      cachedBrowserDocument,
    ]);
    const bindingStore = new MemoryLocalFileBindingStore();
    const posted: DocumentSyncWorkerResponse[] = [];
    const handle = createDocumentSyncWorkerMessageHandler(
    ...
    await handle(
      createLoadRequest(seed, "request_document_sync_browser_only_load"),
    );
  ...
  async function testRepositoryLoadFailureDoesNotFallBackToLinkedCache() {
    ...
    await saveBinding(bindingStore, seed, {
      ...
      async getFile() {
        fileRead = true;
        return new File(
          [createLocalAuthoredDocumentPayload(seed)],
          "unused.cadara",
        );
      },
...
class MemoryLocalFileBindingStore implements LocalFileBindingStore {
  ...
  async save(record: LocalFileBindingRecord) {
    this.saved = this.saved
      .filter(
        (current) => current.metadata.documentId !== record.metadata.documentId,
      )
      .concat(record);
...
class TrackingMemoryDocumentRepository extends MemoryDocumentRepository {
  ...
  async load(input: Parameters<MemoryDocumentRepository["load"]>[0]) {
    ...
    if (result.ok) {
      const overridden = {
        ...result,
        ...(this.loadOverrides.diagnostics
          ? { diagnostics: this.loadOverrides.diagnostics }
          : {}),
        ...(this.loadOverrides.assetAvailability
          ? { assetAvailability: this.loadOverrides.assetAvailability }
          : {}),
        metadata: {
...
class FailingLoadDocumentRepository extends TrackingMemoryDocumentRepository {
  ...
  async load(
    input: Parameters<MemoryDocumentRepository["load"]>[0],
