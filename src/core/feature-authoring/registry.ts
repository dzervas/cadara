...
function isFeatureAuthoringDefinitionForKind<TKind extends AuthoredFeatureKind>(
  ...
  return Boolean(
    ...
    (definition as { metadata?: { kind?: unknown } }).metadata?.kind ===
      featureKind,
  );
}

export function getFeatureAuthoringDefinition<
  TKind extends AuthoredFeatureKind,
>(featureKind: TKind): FeatureAuthoringDefinition<TKind> {
  ...
  throw new Error(
    `Feature authoring definition ${featureKind} resolved to a mismatched feature kind.`,
  );
}

export function findFeatureAuthoringDefinition<
  TKind extends AuthoredFeatureKind,
>(featureKind: TKind): FeatureAuthoringDefinition<TKind> | null {
  const definition = featureAuthoringRegistry.find?.(featureKind);
  return definition &&
    isFeatureAuthoringDefinitionForKind(definition, featureKind)
    ? definition
    : null;
}
