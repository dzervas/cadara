...
function getTargetsForRole(
  draft: LoftFeatureParameterDraft,
...
function isLoftProfileConditionKind(
  value: unknown,
): value is LoftProfileConditionKind {
  return value === "none" || value === "normal" || value === "tangent";
}

function isLoftConnectionTarget(
  value: unknown,
): value is Extract<PrimitiveRef, { kind: "edge" | "vertex" }> {
  return (
    !!value &&
    typeof value === "object" &&
...
function getDefaultLoftProfileConditions(): NonNullable<
  LoftFeatureParameterDraft["options"]["profileConditions"]
> {
  return {
    startCondition: "none",
    startMagnitude: 1,
...
function getLoftProfileConditions(
  options: LoftFeatureParameterDraft["options"],
) {
  return options.profileConditions ?? getDefaultLoftProfileConditions();
}

function filterMatchConnections(
  value: unknown,
): NonNullable<LoftFeatureParameterDraft["options"]["matchConnections"]> {
  ...
  return value.filter(
    (
      ...
      LoftFeatureParameterDraft["options"]["matchConnections"]
    >[number] =>
      !!entry &&
      typeof entry === "object" &&
...
function hydrateLoftProfileConditions(
  value: unknown,
): NonNullable<LoftFeatureParameterDraft["options"]["profileConditions"]> {
...
function updateLoftOptions(
  ...
  const pathPatch =
    patch.path && typeof patch.path === "object" && !Array.isArray(patch.path)
      ? (patch.path as Record<string, unknown>)
      : {};
  const currentProfileConditions = getLoftProfileConditions(current);
  const profileConditionsPatch =
    patch.profileConditions &&
    typeof patch.profileConditions === "object" &&
    !Array.isArray(patch.profileConditions)
  ...
  return {
    ...
    matchConnections:
      patch.matchConnections === undefined
...
function buildLoftParticipants(draft: LoftFeatureParameterDraft) {
  return buildAdvancedSolidParticipants(loftParticipants, (role) =>
...
function moveTarget(
  ...
  const index = target
    ? targets.findIndex((entry) => primitiveRefEquals(entry, target))
    : -1;
  if (index < 0) {
    return targets;
  }
...
export const loftAuthoringDefinition = {
  ...
  hydrateDraft(feature) {
    ...
    return {
      ...
      pathTarget:
        filterTargets(getParticipantTargets("path"), asSweepPathRef)[0] ?? null,
      ...
      options: {
        ...
        path: {
          sectionCount: ((
            feature.parameters.options?.path as
              | { sectionCount?: unknown }
              | undefined
          )?.sectionCount ??
            feature.parameters.options?.sectionCount ??
            5) as NonNullable<
            LoftFeatureParameterDraft["options"]["path"]
          >["sectionCount"],
        },
        profileConditions: hydrateLoftProfileConditions(
          feature.parameters.options?.profileConditions,
        ),
        matchConnections: filterMatchConnections(
          feature.parameters.options?.matchConnections,
        ),
      },
    };
  },
        ...
        patch.profileTargets === undefined
          ? patch.moveProfileTargetEarlier !== undefined
            ? moveTarget(
                ...
                patch.moveProfileTargetEarlier as PrimitiveRef | null,
                -1,
              )
            : patch.moveProfileTargetLater !== undefined
              ? moveTarget(
                  draft.profileTargets,
                  patch.moveProfileTargetLater as PrimitiveRef | null,
                  1,
                )
              : draft.profileTargets
  ...
  applySelection(draft, target) {
    ...
    if (guideCurveTarget) {
      return {
        ...draft,
        guideCurveTargets: appendUniqueTarget(
          draft.guideCurveTargets,
          guideCurveTarget,
        ),
      };
  ...
  getMissingInputsDiagnostics(input) {
    ...
    return [
      createMissingInputDiagnostic({
        ...
        suffix: "references",
        message:
          "Loft preview requires at least two ordered profile references.",
      }),
    ];
  },
          ...
          parameters: {
            ...
            options: {
              ...(draft.pathTarget
                ? {
                    path: {
                      sectionCount: authoredDefinitionValue(
                        pathOptions.sectionCount,
                        5,
                      ),
                    },
                  }
                : {}),
              guideContinuity: authoredDefinitionValue(
                draft.options.guideContinuity ?? "none",
                "none",
              ),
              profileConditions: {
                startCondition: authoredDefinitionValue(
                  profileConditions.startCondition,
                  "none",
                ),
                startMagnitude: authoredDefinitionValue(
                  profileConditions.startMagnitude ?? 1,
                  1,
                ),
                endCondition: authoredDefinitionValue(
                  profileConditions.endCondition,
                  "none",
                ),
                endMagnitude: authoredDefinitionValue(
                  profileConditions.endMagnitude ?? 1,
                  1,
                ),
              },
  ...
  getFormSchema(session) {
    ...
    const guideContinuity = authoredStringLiteral(
      session.draft.options.guideContinuity ?? "none",
      "none",
    );
    const startCondition = authoredStringLiteral(
      profileConditions.startCondition,
      "none",
    );
    const endCondition = authoredStringLiteral(
      profileConditions.endCondition,
      "none",
    );
    const sectionCount = authoredNumberLiteral(pathOptions.sectionCount);
    const sectionCountIsValid =
      isExpressionAuthoredValue(pathOptions.sectionCount) ||
      (sectionCount !== null &&
        Number.isInteger(sectionCount) &&
        sectionCount > 0);
    const startMagnitude = authoredNumberLiteral(
      profileConditions.startMagnitude ?? 1,
    );
    const endMagnitude = authoredNumberLiteral(
      profileConditions.endMagnitude ?? 1,
    );
    const startMagnitudeIsValid =
      isExpressionAuthoredValue(profileConditions.startMagnitude) ||
      (startMagnitude !== null && startMagnitude > 0);
    const endMagnitudeIsValid =
      isExpressionAuthoredValue(profileConditions.endMagnitude) ||
      ...
      sections: [
        {
          ...
          fields: [
            createReferenceCollectionField({
              ...
              helper:
                "Accepted targets: derived sketch regions or planar faces. Order is preserved and drives the loft sections.",
              error:
                session.draft.profileTargets.length >= 2
                  ? null
                  : { message: "Select at least two profile targets." },
            ...
            }),
            {
              ...
              helper:
                "Optional centerline-like path control, stored separately from guide curves.",
              advancedParticipant: {
                role: "path",
                required: false,
                cardinality: { min: 0, max: 1 },
              ...
              fields: [
                {
                  ...
                  authoredValue: {
                    ...
                    source: isExpressionAuthoredValue(pathOptions.sectionCount)
                      ? "expression"
                      : "literal",
                    expressionText: isExpressionAuthoredValue(
                      pathOptions.sectionCount,
                    )
                      ? pathOptions.sectionCount.valueText
                      : null,
                  },
                  error: sectionCountIsValid
                    ? null
                    : { message: "Section count must be a positive integer." },
                  patch:
                    LOFT_ADVANCED_OPTION_DESCRIPTORS[0].options[0].patchTarget,
                },
              ],
            },
            {
              ...
              fields: [
                {
                  ...
                  authoredValue: {
                    ...
                    source: isExpressionAuthoredValue(
                      session.draft.options.guideContinuity,
                    )
                      ? "expression"
                      : "literal",
                    expressionText: isExpressionAuthoredValue(
                      ...
                      ? session.draft.options.guideContinuity.valueText
                      : null,
                  },
              ...
              fields: [
                {
                  ...
                  patch:
                    LOFT_ADVANCED_OPTION_DESCRIPTORS[2].options[0].patchTarget,
                },
                {
                  kind: "numeric",
                  ...
                  authoredValue: {
                    ...
                    source: isExpressionAuthoredValue(
                      profileConditions.startMagnitude,
                    )
                      ? "expression"
                      : "literal",
                    expressionText: isExpressionAuthoredValue(
                      ...
                      ? profileConditions.startMagnitude.valueText
                      : null,
                  },
                  error: startMagnitudeIsValid
                    ? null
                    : { message: "Start magnitude must be positive." },
                  patch:
                    LOFT_ADVANCED_OPTION_DESCRIPTORS[2].options[1].patchTarget,
                },
                {
                  kind: "enum",
                  ...
                  patch:
                    LOFT_ADVANCED_OPTION_DESCRIPTORS[2].options[2].patchTarget,
                },
                {
                  kind: "numeric",
                  ...
                  authoredValue: {
                    ...
                    valueKind: { kind: "positiveNumber" },
                    source: isExpressionAuthoredValue(
                      profileConditions.endMagnitude,
                    )
                      ? "expression"
                      : "literal",
                    expressionText: isExpressionAuthoredValue(
                      ...
                      ? profileConditions.endMagnitude.valueText
                      : null,
                  },
                  error: endMagnitudeIsValid
                    ? null
                    : { message: "End magnitude must be positive." },
                  patch:
                    LOFT_ADVANCED_OPTION_DESCRIPTORS[2].options[3].patchTarget,
                },
              ],
            },
            {
              ...
              helper:
                "Durable edge or vertex connection pairs are preserved for alignment.",
            },
          ],
        },
        {
