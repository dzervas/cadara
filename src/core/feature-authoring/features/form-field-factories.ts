...
export function createBooleanOperationFields(input: {
  ...
  return [
    ...
    },
    {
      ...
      error:
        input.operation === "newBody" || input.booleanTargetBodies.length > 0
          ? null
...
export function createAdvancedOperationIntentFields(input: {
  ...
  return [
    ...
    },
    {
      ...
      emptyLabel: "None selected",
      helper:
        "Add, subtract, and intersect require at least one explicit target body.",
      hidden: input.operationIntent === "create",
      error:
        input.operationIntent === "create" || input.targetBodyTargets.length > 0
          ? null
          : { message: "Select at least one target body." },
      advancedParticipant: {
        role: "targetBody",
        ...
        cardinality: {
          min: input.operationIntent === "create" ? 0 : 1,
          max: null,
        },
