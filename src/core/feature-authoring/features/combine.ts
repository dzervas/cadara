...
function isCombineOperationIntent(
  value: unknown,
): value is CombineOperationIntent {
...
function getTargetsForRole(
  draft: CombineFeatureParameterDraft,
...
function getCombineValidationDiagnostics(draft: CombineFeatureParameterDraft) {
  ...
  for (const targetBody of draft.targetBodyTargets) {
    if (
      draft.toolBodyTargets.some((toolBody) =>
        primitiveRefEquals(toolBody, targetBody),
      )
    ) {
      diagnostics.push({
...
export const combineAuthoringDefinition = {
  ...
  applySelection(draft, target) {
    ...
    if (
      !draft.targetBodyTargets.some((entry) =>
        primitiveRefEquals(entry, bodyTarget),
      )
    ) {
      return this.applyPatch(draft, {
        toolBodyTargets: appendUniqueTarget(draft.toolBodyTargets, bodyTarget),
      });
    }
  ...
  getMissingInputsDiagnostics(input) {
    ...
    return [
      createMissingInputDiagnostic({
        ...
        suffix: "references",
        message:
          "Combine preview requires target bodies, tool bodies, and a boolean operation.",
      }),
    ];
  },
  buildDefinition(draft) {
    return getCombineValidationDiagnostics(draft).length === 0
      ? buildCombineDefinition(draft)
      : null;
  },
      ...
      sections: [
        {
          ...
          fields: [
            ...
            },
            {
              ...
              error:
                session.draft.toolBodyTargets.length > 0
                  ? null
                  : { message: "Select at least one tool body." },
              advancedParticipant: {
                role: "toolBody",
                required: true,
                cardinality: { min: 1, max: null },
