...
function isEndConditionKind(
  value: unknown,
): value is RevolveEndCondition["kind"] {
  return (
    value === "full" ||
    value === "blind" ||
    value === "upToNext" ||
...
function patchEnd(
  ...
  const offsetAngleKey =
    prefix === "first" ? "upToOffsetAngle" : "secondUpToOffsetAngle";
  ...
  const nextKind = isEndConditionKind(patch[conditionKey])
    ? patch[conditionKey]
    : end.kind;
  const direction = isRevolveDirection(patch[directionKey])
    ? patch[directionKey]
    : "direction" in end
      ? end.direction
      : "counterClockwise";
  ...
  const offsetDirection = isOffsetDirection(patch[offsetDirectionKey])
    ...
    : "offset" in end
      ...
      : "shorten";
  const offset = isFiniteAuthoredNumber(offsetAngle)
    ? { angle: offsetAngle, direction: offsetDirection }
    : undefined;
  ...
  if (nextKind === "blind") {
    return {
      ...
      angle: acceptAuthoredPatch(
        patch[angleKey],
        end.kind === "blind" ? end.angle : Math.PI * 2,
        (value): value is number => typeof value === "number",
      ),
    };
  }
  ...
  if (nextKind === "upToNext") {
    return offset
      ? { kind: "upToNext", direction, offset }
      : { kind: "upToNext", direction };
  }
...
function endHasValidScalars(end: RevolveFeatureEndConditionDraft) {
  return (
    ...
    (!("offset" in end) ||
      ...
      isFiniteAuthoredNumber(end.offset.angle))
  );
}
      ...
      return {
        ...
        ...(end.offset
          ? {
              offset: {
                ...end.offset,
                angle: authoredDefinitionValue(end.offset.angle, 0),
              },
            }
          : {}),
...
function endFields(
  ...
  const fields: FeatureEditorFormField[] = [
    {
      ...
      options: options.map((value) => ({
        value,
        label: endConditionLabel(value as RevolveEndCondition["kind"]),
      })),
  ...
  if (end.kind === "blind") {
    fields.push({
      ...
      authoredValue: expressionCapableAuthoredValue(end.angle, {
        kind: "positiveNumber",
      }),
      error: isPositiveAuthoredNumber(end.angle)
        ? null
        : { message: "Angle must be greater than zero." },
    ...
    fields.push(
      {
        ...
        input: "angleDegrees",
        step: 1,
        authoredValue: expressionCapableAuthoredValue(end.offset?.angle ?? 0, {
          kind: "angle",
        }),
        error:
          end.offset?.angle === undefined ||
          isFiniteAuthoredNumber(end.offset.angle)
            ? null
            : { message: "Offset angle must be finite." },
...
export const revolveAuthoringDefinition = {
    ...
    return {
      ...
      axisTarget:
        patch.axisTarget === undefined
          ? draft.axisTarget
          : asRevolveAxisRef(
              patch.axisTarget as Parameters<typeof asRevolveAxisRef>[0],
            ),
      startAngle: acceptAuthoredPatch(
        patch.startAngle,
        draft.startAngle,
        (value): value is number => typeof value === "number",
      ),
    ...
    if (input.draft.profileTargets.length === 0 || !input.draft.axisTarget) {
      diagnostics.push(
        createMissingInputDiagnostic({
          ...
          suffix: "references",
          message:
            "Revolve preview requires both a profile and an axis reference.",
        }),
      );
    }
    ...
    if (
      ...
      diagnostics.push(
        createMissingInputDiagnostic({
          ...
          message:
            "Complete the active revolve end condition before previewing revolve.",
        }),
      );
    }
    ...
    if (
      ...
      diagnostics.push(
        createMissingInputDiagnostic({
          ...
          message:
            "Complete the second revolve end condition before previewing revolve.",
        }),
      );
    }
  ...
  buildDefinition(draft) {
    ...
    const extent: RevolveFeatureExtent =
      draft.extentMode === "twoSide"
        ? {
            mode: "twoSide",
            firstEnd: definitionEnd(firstEnd) as Exclude<
              RevolveEndCondition,
              { kind: "full" }
            >,
            secondEnd: definitionEnd(draft.secondEnd) as Exclude<
              RevolveEndCondition,
              { kind: "full" }
            >,
          }
      ...
      sections: [
        {
          ...
          fields: [
            ...
            },
            {
              ...
              emptyLabel: "None selected",
              helper:
                "Accepted targets: one durable edge or one construction axis.",
              error: session.draft.axisTarget
                ? null
                : { message: "Select a revolve axis." },
              advancedParticipant: {
                role: "axis",
                required: true,
                cardinality: { min: 1, max: 1 },
