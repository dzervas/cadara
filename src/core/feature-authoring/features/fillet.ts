      ...
      edgeTargets:
        patch.edgeTargets === undefined && patch.edgeTarget === undefined
          ...
          : Array.isArray(patch.edgeTargets)
            ? patch.edgeTargets.filter(
                (
                  entry,
                ): entry is Extract<
                  (typeof draft.edgeTargets)[number],
                  { kind: "edge" }
                > => entry?.kind === "edge",
              )
          ...
          fields: [
            {
              ...
              authoredValue: expressionCapableAuthoredValue(
                session.draft.radius,
                { kind: "positiveNumber" },
              ),
              error: isPositiveAuthoredNumber(session.draft.radius)
                ? null
