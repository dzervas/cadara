...
function buildSplitDefinition(draft: SplitFeatureParameterDraft) {
  return {
    ...
    parameters: {
      participants: [
        ...(draft.targetBodyTarget
          ? [{ role: "targetBody" as const, targets: [draft.targetBodyTarget] }]
          : []),
        ...(draft.toolBodyTarget
          ? [{ role: "toolBody" as const, targets: [draft.toolBodyTarget] }]
          : []),
      ],
    },
  };
...
export const splitAuthoringDefinition = {
  ...
  applyPatch(draft, patch) {
    return {
      ...draft,
      targetBodyTarget:
        patch.targetBodyTarget === undefined
          ? draft.targetBodyTarget
          : asBodyRef(patch.targetBodyTarget as PrimitiveRef | null),
      toolBodyTarget:
        patch.toolBodyTarget === undefined
          ? draft.toolBodyTarget
          : asBodyRef(patch.toolBodyTarget as PrimitiveRef | null),
    };
  },
  applySelection(draft, target) {
    ...
    if (
      !draft.toolBodyTarget &&
      draft.targetBodyTarget.bodyId !== bodyTarget.bodyId
    ) {
  ...
  getMissingInputsDiagnostics(input) {
    ...
    return [
      createMissingInputDiagnostic({
        ...
        suffix: "references",
        message:
          "Split preview requires one target body and one split tool body.",
      }),
    ];
  },
  buildDefinition(draft) {
    return getSplitValidationDiagnostics(draft).length === 0
      ? buildSplitDefinition(draft)
      : null;
  },
          ...
          fields: [
            {
              ...
              error: session.draft.targetBodyTarget
                ? null
                : { message: "Select one target body." },
              advancedParticipant: {
                role: "targetBody",
                required: true,
                cardinality: { min: 1, max: 1 },
