...
import { createFeatureEditorFieldPatch } from "@/core/feature-authoring/form-events";
import type {
...
export type FeatureEditorExpressionField = Extract<
  FeatureEditorFormField,
  { kind: "numeric" | "enum" }
>;
...
export type FeatureEditorFormValue =
...
export function getFeatureEditorInputFields(
  schema: FeatureEditorFormSchema,
): FeatureEditorPatchableField[] {
  return schema.sections.flatMap((section) =>
    section.fields.flatMap(getFeatureEditorInputFieldsFromField),
  );
}

export function createFeatureEditorFormValues(
  schema: FeatureEditorFormSchema,
...
export function normalizeFeatureEditorFormValues(
  ...
  for (const field of getFeatureEditorInputFields(schema)) {
    normalized[field.id] = normalizeFeatureEditorFieldValue(
      field,
      values[field.id],
    );
  }
...
export function shouldResetFeatureEditorFormValues(input: {
  ...
  if (
    featureEditorFormValuesEqual(
      input.schema,
      input.nextValues,
      input.lastSyncedValues,
    )
  ) {
    return false;
  }

  return !featureEditorFormValuesEqual(
    input.schema,
    input.currentValues,
    input.nextValues,
  );
  ...
  switch (field.kind) {
    case "numeric": {
      ...
      if (controlValue.source === "expression") {
        return createFeatureEditorPatchFromExpression(
          field,
          controlValue.expressionText ?? "",
        );
      }
    ...
    case "referencePicker":
      return createFeatureEditorFieldPatch(
        field,
        normalizeReferenceFormValue(value),
      );
    case "referenceCollection":
      return createFeatureEditorFieldPatch(
        field,
        normalizeReferenceCollectionFormValue(value),
      );
  }
}
...
export function getFeatureEditorExpressionSourceState(
  ...
  return field.authoredValue?.expressionCapable
    ? normalizeControlFormValue(field, value)
    : null;
}
...
export function previewFeatureEditorFieldExpression(input: {
  ...
  const formValue = formatResolvedExpressionFormValue(
    input.field,
    preview.value,
  );
  return {
...
function getFeatureEditorFieldValue(
  field: FeatureEditorPatchableField,
...
function createControlFormValue(
  ...
  const source =
    field.authoredValue?.source === "expression" ? "expression" : "literal";
...
function normalizeControlFormValue(
    ...
    return {
      ...
      expressionText:
        value.source === "expression"
          ? (value.expressionText ?? value.value)
          : null,
    };
  }

  if (isExpressionAuthoredValue(value)) {
    return createFeatureEditorExpressionControlFormValue(
      value.valueText,
      value.valueText,
    );
  }
  ...
  return createControlFormValue(
    field,
    field.kind === "numeric" ? String(field.value) : field.value,
  );
}

function isFeatureEditorControlFormValue(
  value: unknown,
): value is FeatureEditorControlFormValue {
  return (
    typeof value === "object" &&
    value !== null &&
    !Array.isArray(value) &&
    (value as { formValueKind?: unknown }).formValueKind ===
      "featureEditorControl" &&
...
function numericFormValuesEqual(
...
function formatResolvedExpressionFormValue(
  field: FeatureEditorExpressionField,
  value: unknown,
): string {
  if (field.kind === "numeric") {
    ...
    return String(
      field.input === "angleDegrees" ? value * (180 / Math.PI) : value,
    );
  }
...
function referenceFormValuesEqual(
  left: FeatureEditorFormValue,
  right: FeatureEditorFormValue,
): boolean {
  ...
  return (
    isPrimitiveRef(left) &&
    isPrimitiveRef(right) &&
    primitiveRefEquals(left, right)
  );
}

function referenceCollectionFormValuesEqual(
  left: FeatureEditorFormValue,
  right: FeatureEditorFormValue,
): boolean {
  if (
...
function getFeatureEditorInputFieldsFromField(
  ...
  switch (field.kind) {
    ...
    case "discriminatedOptionGroup": {
      const activeVariant = field.variants.find(
