...
export interface SweepFeatureParameterDraft {
  ...
  guideCurveTargets: readonly Extract<
    PrimitiveRef,
    { kind: "edge" | "sketchEntity" }
  >[];
  lockProfileFaceTargets: readonly Extract<PrimitiveRef, { kind: "face" }>[];
  lockProfileDirectionTarget: Extract<
    PrimitiveRef,
    { kind: "edge" | "construction" }
  > | null;
...
export interface CombineFeatureParameterDraft {
  ...
  operationIntent: MaybeAuthoredValue<
    Exclude<AdvancedSolidOperationIntent, "create">
  >;
}
...
export interface TransformFeatureParameterDraft {
  bodyTargets: readonly Extract<PrimitiveRef, { kind: "body" }>[];
  transformReferenceTarget: Extract<
    PrimitiveRef,
    { kind: "construction" | "face" }
  > | null;
  distance: MaybeAuthoredValue<number>;
...
export type FeatureEditSessionStateForKind<TKind extends AuthoredFeatureKind> =
...
export type ExtrudeFeatureEditSessionState =
  FeatureEditSessionStateForKind<"extrude">;
export type RevolveFeatureEditSessionState =
  FeatureEditSessionStateForKind<"revolve">;
export type FilletFeatureEditSessionState =
  FeatureEditSessionStateForKind<"fillet">;
export type PlaneFeatureEditSessionState =
  FeatureEditSessionStateForKind<"plane">;
export type ShellFeatureEditSessionState =
  FeatureEditSessionStateForKind<"shell">;
export type SweepFeatureEditSessionState =
  FeatureEditSessionStateForKind<"sweep">;
export type LoftFeatureEditSessionState =
...
export type CombineFeatureEditSessionState =
  FeatureEditSessionStateForKind<"combine">;
export type SplitFeatureEditSessionState =
  FeatureEditSessionStateForKind<"split">;
export type DeleteSolidFeatureEditSessionState =
  FeatureEditSessionStateForKind<"deleteSolid">;
export type MirrorFeatureEditSessionState =
...
export interface FeatureAuthoringMetadata<
  TKind extends AuthoredFeatureKind = AuthoredFeatureKind,
...
export interface FeatureAuthoringDefinition<
  TKind extends AuthoredFeatureKind = AuthoredFeatureKind,
> {
  ...
  hydrateDraft(
    feature: FeatureDefinitionByKind<TKind>,
  ): FeatureDraftByKind[TKind];
  applyPatch(
    draft: FeatureDraftByKind[TKind],
  ...
  buildDefinition(
    draft: FeatureDraftByKind[TKind],
  ): FeatureDefinitionByKind<TKind> | null;
  getFormSchema(
...
export function isBooleanOperation(
  value: unknown,
): value is FeatureBooleanOperation {
  return (
    value === "newBody" ||
    value === "join" ||
    value === "cut" ||
...
export function toBodyIds(value: unknown): readonly BodyId[] | null {
  ...
  const bodyIds = value.map((entry) => {
    ...
    return entry &&
      ...
      entry.kind === "body" &&
      "bodyId" in entry &&
      typeof entry.bodyId === "string"
      ? (entry.bodyId as BodyId)
      : null;
  });

  return bodyIds.some((entry) => entry === null)
    ? null
    : (bodyIds as readonly BodyId[]);
}

export function toBooleanScope(
  patch: FeatureDraftPatch,
  current: FeatureBooleanScope,
): FeatureBooleanScope {
  if (
    patch.booleanScope &&
    typeof patch.booleanScope === "object" &&
    patch.booleanScope !== null
  ) {
    const next = patch.booleanScope as Partial<FeatureBooleanScope> & {
      kind?: string;
    };
    ...
    if (
      next.kind === "targetBody" &&
      typeof (next as { bodyId?: unknown }).bodyId === "string"
    ) {
      return {
        kind: "targetBody",
        bodyId: (next as { bodyId: BodyId }).bodyId,
      };
    }
