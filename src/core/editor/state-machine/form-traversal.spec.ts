...
test("form-traversal.ts finds nested form fields in option groups and discriminated variants", () => {
  ...
  const nestedDiscriminant = findNestedFormFieldById(
...
test("form-traversal.ts resolves the active feature reference picker only for top-level reference fields", () => {
  ...
  const referenceField = findFirstTopLevelFieldOfKind(schema, [
    "referencePicker",
    "referenceCollection",
  ]);
  const nonReferenceField = findFirstTopLevelFieldOfKind(schema, [
    "numeric",
    "enum",
    "summary",
    "custom",
...
test("form-traversal.ts chooses a default import selection field only when exactly one visible reference field remains", () => {
  ...
  const defaultField = getDefaultImportSelectionField(
    makeImportSession(schemaWithHiddenCompanion),
  );
  const ambiguousField = getDefaultImportSelectionField(
...
test("form-traversal.ts resolves active import picker fields and patches viewport selections with reopened sketch planes", async () => {
  ...
  const pickerField = makeReferencePickerField(
    "sketch-target",
    "selectionPatch",
  );
  const collectionField = makeReferenceCollectionField(
    "sketch-targets",
    "collectionPatch",
  );
  const state = {
  ...
  const pickerPatch = createImportViewportSelectionPatch(
    state,
    activeField,
    sketchTarget,
  );
