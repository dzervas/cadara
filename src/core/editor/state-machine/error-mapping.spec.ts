...
test("error-mapping.ts extracts effect context for feature, sketch, and special-mode effects", () => {
  ...
  expectTrue(
    featureContext.some(
      ...
      featureContext.some(
        (entry) =>
          entry.key === "featureId" && entry.value === featureSession.featureId,
      ) &&
      featureContext.some(
        (entry) =>
  ...
  expectTrue(
    specialModeContext.some(
      ...
      specialModeContext.some(
        (entry) => entry.key === "effectId" && entry.value === "replace-image",
      ) &&
      specialModeContext.some(
        (entry) =>
          entry.key === "effectKind" &&
...
test("error-mapping.ts maps each editor effect type to its typed failure event seam", () => {
  ...
  for (const { effect, expectedType, assertResult } of failureCases) {
    const event = createEditorEffectFailureEvent(
      effect,
...
test("error-mapping.ts filters non-durable targets and converts modeling mutation app errors into diagnostics", () => {
  ...
  const diagnostic = modelingMutationErrorToDiagnostic(
    modelingError,
    durableTarget,
  );
