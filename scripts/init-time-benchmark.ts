...
const DOCUMENT_REPOSITORY_URL_STORAGE_KEY =
  "cad.documentRepository.automergeUrls.v1";
const SERVER_TIMEOUT_MS = 120_000;
const LOAD_TIMEOUT_MS = 120_000;
const CUSTOM_OCC_ASSET_BASENAMES = [
  "cadara-occ.js",
  "cadara-occ.wasm",
] as const;
...
async function waitForRestoredGeometry(page: Page) {
  ...
  if (state !== "ready") {
    ...
    console.error(
      "Preview build failed OCC readiness probe. Body text snapshot:",
    );
    console.error(bodyText.slice(0, 4000));
    throw new Error(
      `Preview build did not reach OCC-ready state: ${String(state)}`,
    );
  }
...
async function main() {
  if (!existsSync("/app/dist/index.html")) {
    throw new Error(
      "Preview build is missing. Run `bun run build` once a valid build baseline is available.",
    );
  }
  ...
  try {
    ...
    try {
      ...
      const mutationPerf = await page.evaluate(
        async () => window.__cadMeasureOccMutation?.() ?? null,
      );
        ...
        JSON.stringify(
          {
            ...
            browserElapsedMs:
              browserElapsedMs === null
                ? null
                : Number(browserElapsedMs.toFixed(1)),
            occWarmupMs:
              occPerf?.warmupStartedAt != null &&
              occPerf?.warmupSettledAt != null
                ? Number(
                    (occPerf.warmupSettledAt - occPerf.warmupStartedAt).toFixed(
                      1,
                    ),
                  )
                : null,
            ...
            readinessSignal:
              "window.__cadOccPerf.firstNonEmptyGeometryFrameAt || firstSnapshotReadyAt + 2 animation frames",
          },
          null,
          2,
        ),
